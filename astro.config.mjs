...
function readGeneratedSidebarSections() {
  try {
    const raw = readFileSync(
      new URL("./src/config/sidebar.generated.json", import.meta.url),
      "utf-8",
    );
...
function mergeSidebarWithGenerated(staticSidebar, generatedSidebar) {
  ...
  for (const generatedSection of generatedSidebar) {
    ...
    const index = merged.findIndex(
      (item) => extractSectionKey(item) === generatedKey,
    );
...
gtag('config', '${GA_MEASUREMENT_ID}');`,
      ...
      sidebar: mergeSidebarWithGenerated(
        staticSidebar,
        generatedSidebarSections,
      ),
    }),
    sitemap(),
  ],
