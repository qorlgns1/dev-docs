---
title: '구성 참조'
description: '이 페이지는 Codex 구성 파일을 위한 검색 가능한 참조입니다. 개념적 안내와 예제를 보려면 Config basics와 Advanced Config부터 시작하세요.'
---

출처 URL: https://developers.openai.com/codex/config-reference

# 구성 참조

이 페이지는 Codex 구성 파일을 위한 검색 가능한 참조입니다. 개념적 안내와 예제를 보려면 [Config basics](https://developers.openai.com/codex/config-basic)와 [Advanced Config](https://developers.openai.com/codex/config-advanced)부터 시작하세요.

## `config.toml`

사용자 수준 구성은 `~/.codex/config.toml`에 저장됩니다. 프로젝트 범위의 오버라이드는 `.codex/config.toml` 파일에서 추가할 수 있습니다. Codex는 해당 프로젝트를 신뢰하는 경우에만 프로젝트 범위 구성 파일을 로드합니다.

샌드박스 및 승인 키(`approval_policy`, `sandbox_mode`, `sandbox_workspace_write.*`)에 대해서는 [Sandbox and approvals](https://developers.openai.com/codex/security#sandbox-and-approvals), [Protected paths in writable roots](https://developers.openai.com/codex/security#protected-paths-in-writable-roots), [Network access](https://developers.openai.com/codex/security#network-access) 문서를 함께 참고하세요.
...
`config.toml`의 최신 JSON 스키마는 [여기](https://developers.openai.com/codex/config-schema.json)에서 확인할 수 있습니다.

VS Code나 Cursor에서 `config.toml`을 편집할 때 자동 완성과 진단을 받으려면 [Even Better TOML](https://marketplace.visualstudio.com/items?itemName=tamasfe.even-better-toml) 확장 기능을 설치하고, `config.toml` 상단에 다음 줄을 추가하세요:

```toml
#:schema https://developers.openai.com/codex/config-schema.json
```

참고: `experimental_instructions_file`의 이름을 `model_instructions_file`로 바꾸세요. Codex가 이전 키를 더 이상 사용하지 않으므로 기존 구성도 새 이름으로 업데이트해야 합니다.

## `requirements.toml`

`requirements.toml`은 사용자가 재정의할 수 없는 보안 민감 설정을 제한하는 관리자 적용형 구성 파일입니다. 자세한 정보, 위치, 예시는 [관리자 적용 요구사항](https://developers.openai.com/codex/security#admin-enforced-requirements-requirementstoml)을 참고하세요.

ChatGPT Business 및 Enterprise 사용자의 경우 Codex가 클라우드에서 가져온 requirements를 적용할 수도 있습니다. 우선순위 상세는 보안 페이지를 확인하세요.
