---
title: 'next.config.js 옵션: experimental.adapterPath'
description: 'Next.js는 빌드 프로세스에 연결할 사용자 정의 어댑터를 생성할 수 있는 실험적 API를 제공합니다. 이는 Next.js 구성을 수정하거나 빌드 출력을 처리해야 하는 배포 플랫폼 또는 맞춤형 빌드 통합에 유용합니다.'
---

# next.config.js 옵션: experimental.adapterPath | Next.js
...
Next.js는 빌드 프로세스에 연결할 사용자 정의 어댑터를 생성할 수 있는 실험적 API를 제공합니다. 이는 Next.js 구성을 수정하거나 빌드 출력을 처리해야 하는 배포 플랫폼 또는 맞춤형 빌드 통합에 유용합니다.

## 구성[](https://nextjs.org/docs/pages/api-reference/config/next-config-js/adapterPath#configuration)

어댑터를 사용하려면 `experimental.adapterPath`에 어댑터 모듈 경로를 지정하세요:
...
### 기본 어댑터 구조[](https://nextjs.org/docs/pages/api-reference/config/next-config-js/adapterPath#basic-adapter-structure)
...
## API 참조[](https://nextjs.org/docs/pages/api-reference/config/next-config-js/adapterPath#api-reference)
...
**매개변수:**
  ...
  * `routes`: 헤더, 리다이렉트, 리라이트, 동적 라우트에 대한 매니페스트를 포함하는 객체
    * `routes.headers`: `source`, `sourceRegex`, `headers`, `has`, `missing`, 선택적 `priority` 필드를 가진 헤더 라우트 객체 배열
    * `routes.redirects`: `source`, `sourceRegex`, `destination`, `statusCode`, `has`, `missing`, 선택적 `priority` 필드를 가진 리다이렉트 라우트 객체 배열
    * `routes.rewrites`: `beforeFiles`, `afterFiles`, `fallback` 배열로 구성된 객체로, 각 배열에는 `source`, `sourceRegex`, `destination`, `has`, `missing` 필드를 가진 리라이트 라우트 객체가 포함됨
    * `routes.dynamicRoutes`: `source`, `sourceRegex`, `destination`, `has`, `missing` 필드를 가진 동적 라우트 객체 배열
  * `outputs`: 유형별로 정리된 모든 빌드 출력에 대한 세부 정보
  * `projectDir`: Next.js 프로젝트 디렉터리의 절대 경로
  * `repoRoot`: 감지된 저장소 루트의 절대 경로
  * `distDir`: 빌드 출력 디렉터리의 절대 경로
  * `config`: `modifyConfig`가 적용된 최종 Next.js 구성
...
## 라우트 정보[](https://nextjs.org/docs/pages/api-reference/config/next-config-js/adapterPath#routes-information)

`onBuildComplete`의 `routes` 객체는 배포에 바로 사용할 수 있는 처리된 패턴과 함께 완전한 라우팅 정보를 제공합니다.

### 헤더[](https://nextjs.org/docs/pages/api-reference/config/next-config-js/adapterPath#headers)
...
각 리다이렉트 라우트에는 다음이 포함됩니다:
  ...
  * `destination`: 대상 URL(캡처된 그룹을 포함할 수 있음)
...
### 동적 라우트[](https://nextjs.org/docs/pages/api-reference/config/next-config-js/adapterPath#dynamic-routes)

동적 라우트 세그먼트(예: `[slug]`, `[...path]`)에서 생성됩니다. 각 항목은 다음을 포함합니다:

  * `source`: 라우트 패턴
  * `sourceRegex`: 명명된 캡처 그룹이 있는 컴파일된 정규식
  * `destination`: 매개변수 치환이 적용된 내부 목적지
  * `has`: 선택적 긍정 조건
  * `missing`: 선택적 부정 조건
...
어댑터의 일반적인 사용 사례는 다음과 같습니다:
  ...
  * **배포 플랫폼 통합** : 특정 호스팅 플랫폼에 맞춰 빌드 출력 자동 구성
