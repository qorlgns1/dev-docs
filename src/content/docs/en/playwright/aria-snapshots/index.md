...
description: "With Playwright's Snapshot testing you can assert the accessibility tree of a page against a predefined snapshot template."
---

Source URL: https://playwright.dev/docs/aria-snapshots

# Snapshot testing | Playwright

## Overview[​](https://playwright.dev/docs/aria-snapshots#overview "Direct link to Overview")
...
## Assertion testing vs Snapshot testing[​](https://playwright.dev/docs/aria-snapshots#assertion-testing-vs-snapshot-testing "Direct link to Assertion testing vs Snapshot testing")

Snapshot testing and assertion testing serve different purposes in test automation:

### Assertion testing[​](https://playwright.dev/docs/aria-snapshots#assertion-testing "Direct link to Assertion testing")

Assertion testing is a targeted approach where you assert specific values or conditions about elements or components. For instance, with Playwright, [expect(locator).toHaveText()](https://playwright.dev/docs/api/class-locatorassertions#locator-assertions-to-have-text) verifies that an element contains the expected text, and [expect(locator).toHaveValue()](https://playwright.dev/docs/api/class-locatorassertions#locator-assertions-to-have-value) confirms that an input field has the expected value. Assertion tests are specific and generally check the current state of an element or property against an expected, predefined state. They work well for predictable, single-value checks but are limited in scope when testing the broader structure or variations.
...
### Snapshot testing[​](https://playwright.dev/docs/aria-snapshots#snapshot-testing "Direct link to Snapshot testing")
...
### When to use[​](https://playwright.dev/docs/aria-snapshots#when-to-use "Direct link to When to use")
...
## Aria snapshots[​](https://playwright.dev/docs/aria-snapshots#aria-snapshots "Direct link to Aria snapshots")
...
## Snapshot matching[​](https://playwright.dev/docs/aria-snapshots#snapshot-matching "Direct link to Snapshot matching")

The [expect(locator).toMatchAriaSnapshot()](https://playwright.dev/docs/api/class-locatorassertions#locator-assertions-to-match-aria-snapshot) assertion method in Playwright compares the accessible structure of the locator scope with a predefined aria snapshot template, helping validate the page's state against testing requirements.
...
### Partial matching[​](https://playwright.dev/docs/aria-snapshots#partial-matching "Direct link to Partial matching")
...
### Strict matching[​](https://playwright.dev/docs/aria-snapshots#strict-matching "Direct link to Strict matching")
...
### Matching with regular expressions[​](https://playwright.dev/docs/aria-snapshots#matching-with-regular-expressions "Direct link to Matching with regular expressions")
...
## Generating snapshots[​](https://playwright.dev/docs/aria-snapshots#generating-snapshots "Direct link to Generating snapshots")
...
### Generating snapshots with the Playwright code generator[​](https://playwright.dev/docs/aria-snapshots#generating-snapshots-with-the-playwright-code-generator "Direct link to Generating snapshots with the Playwright code generator")

If you're using Playwright's [Code Generator](https://playwright.dev/docs/codegen), generating aria snapshots is streamlined with its interactive interface:
...
### Updating snapshots with `@playwright/test` and the `--update-snapshots` flag[​](https://playwright.dev/docs/aria-snapshots#updating-snapshots-with-playwrighttest-and-the---update-snapshots-flag "Direct link to updating-snapshots-with-playwrighttest-and-the---update-snapshots-flag")
...
#### Empty template for snapshot generation[​](https://playwright.dev/docs/aria-snapshots#empty-template-for-snapshot-generation "Direct link to Empty template for snapshot generation")
...
#### Snapshot patch files[​](https://playwright.dev/docs/aria-snapshots#snapshot-patch-files "Direct link to Snapshot patch files")
...
#### Snapshots as separate files[​](https://playwright.dev/docs/aria-snapshots#snapshots-as-separate-files "Direct link to Snapshots as separate files")
...
By default, snapshots from a test file `example.spec.ts` are placed in the `example.spec.ts-snapshots` directory. As snapshots should be the same across browsers, only one snapshot is saved even if testing with multiple browsers. Should you wish, you can customize the [snapshot path template](https://playwright.dev/docs/api/class-testconfig#test-config-snapshot-path-template) using the following configuration:
...
### Using the `Locator.ariaSnapshot` method[​](https://playwright.dev/docs/aria-snapshots#using-the-locatorariasnapshot-method "Direct link to using-the-locatorariasnapshot-method")

The [locator.ariaSnapshot()](https://playwright.dev/docs/api/class-locator#locator-aria-snapshot) method allows you to programmatically create a YAML representation of accessible elements within a locator's scope, especially helpful for generating snapshots dynamically during test execution.
...
## Accessibility tree examples[​](https://playwright.dev/docs/aria-snapshots#accessibility-tree-examples "Direct link to Accessibility tree examples")

### Headings with level attributes[​](https://playwright.dev/docs/aria-snapshots#headings-with-level-attributes "Direct link to Headings with level attributes")
...
### Text nodes[​](https://playwright.dev/docs/aria-snapshots#text-nodes "Direct link to Text nodes")
...
### Inline multiline text[​](https://playwright.dev/docs/aria-snapshots#inline-multiline-text "Direct link to Inline multiline text")
...
### Links[​](https://playwright.dev/docs/aria-snapshots#links "Direct link to Links")
...
### Text boxes[​](https://playwright.dev/docs/aria-snapshots#text-boxes "Direct link to Text boxes")
...
### Lists with items[​](https://playwright.dev/docs/aria-snapshots#lists-with-items "Direct link to Lists with items")
...
### Grouped elements[​](https://playwright.dev/docs/aria-snapshots#grouped-elements "Direct link to Grouped elements")
...
### Attributes and states[​](https://playwright.dev/docs/aria-snapshots#attributes-and-states "Direct link to Attributes and states")

Commonly used ARIA attributes, like `checked`, `disabled`, `expanded`, `level`, `pressed`, and `selected`, represent control states.

#### Checkbox with `checked` attribute[​](https://playwright.dev/docs/aria-snapshots#checkbox-with-checked-attribute "Direct link to checkbox-with-checked-attribute")
...
#### Button with `pressed` attribute[​](https://playwright.dev/docs/aria-snapshots#button-with-pressed-attribute "Direct link to button-with-pressed-attribute")
