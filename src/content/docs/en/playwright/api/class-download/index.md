---
title: "Download"
description: "All the downloaded files belonging to the browser context are deleted when the browser context is closed."
---
...
[Download](https://playwright.dev/docs/api/class-download "Download") objects are dispatched by page via the [page.on('download')](https://playwright.dev/docs/api/class-page#page-event-download) event.
...
If you don't need a readable stream, it's usually simpler to read the file from disk after the download completed. See [download.path()](https://playwright.dev/docs/api/class-download#download-path).
...
Note that the download's file name is a random GUID, use [download.suggestedFilename()](https://playwright.dev/docs/api/class-download#download-suggested-filename) to get suggested file name.
