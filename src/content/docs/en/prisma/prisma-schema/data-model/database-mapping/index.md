...
Source URL: https://docs.prisma.io/docs/orm/prisma-schema/data-model/database-mapping

# Database mapping

Learn how to map model and field names to database tables and columns

The [Prisma schema](https://docs.prisma.io/docs/orm/prisma-schema/overview) includes mechanisms that allow you to define names of certain database objects. You can:

- Map model and field names to different collection/table and field/column names
- Define constraint and index names
...
A common approach for naming tables/collections in databases for example is to use plural form and [snake_case](https://en.wikipedia.org/wiki/Snake_case) notation. However, we recommended a different [naming convention (singular form, PascalCase)](https://docs.prisma.io/docs/orm/reference/prisma-schema-reference#naming-conventions).

`@map` and `@@map` allow you to [tune the shape of your Prisma Client API](https://docs.prisma.io/docs/orm/prisma-client/setup-and-configuration/custom-model-and-field-names) by decoupling model and field names from table and column names in the underlying database.

- Map collection / table names

As an example, when you [introspect](https://docs.prisma.io/docs/orm/prisma-schema/introspection) a database with a table named `comments`, the resulting Prisma model will look like this:
...
However, you can still choose `Comment` as the name of the model (e.g. to follow the naming convention) without renaming the underlying `comments` table in the database by using the [`@@map`](https://docs.prisma.io/docs/orm/reference/prisma-schema-reference) attribute:
...
You can also [`@map`](https://docs.prisma.io/docs/orm/reference/prisma-schema-reference#map) a column/field name:
...
When no explicit names are provided via `map` arguments Prisma ORM will generate index and constraint names following the [default naming convention](https://docs.prisma.io/docs/orm/prisma-schema/data-model/database-mapping#prisma-orms-default-naming-conventions-for-indexes-and-constraints).
