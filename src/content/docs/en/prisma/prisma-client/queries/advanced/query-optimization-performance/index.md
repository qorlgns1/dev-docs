...
Source URL: https://docs.prisma.io/docs/orm/prisma-client/queries/advanced/query-optimization-performance
...
[Prisma Optimize](https://docs.prisma.io/docs/optimize) provides [recommendations](https://docs.prisma.io/docs/optimize/recommendations) to address these issues. Follow the [integration guide](https://docs.prisma.io/docs/optimize/getting-started) to get started.
...
For serverless development environments with frameworks that use HMR (Hot Module Replacement), ensure you properly handle a [single instance of Prisma in development](https://docs.prisma.io/docs/orm/more/troubleshooting/nextjs#best-practices-for-using-prisma-client-in-development).
...
Use `findUnique()` in combination with [the fluent API](https://docs.prisma.io/docs/orm/prisma-client/queries/relation-queries#fluent-api) (`.posts()`) as shown to return a user's posts. Even though the resolver is called once per user, the Prisma dataloader in Prisma Client **✔ batches the`findUnique()` queries**.
...
You can perform the query with a [database join](https://docs.prisma.io/docs/orm/prisma-client/queries/relation-queries#relation-load-strategies-preview) by setting `relationLoadStrategy` to `"join"`, ensuring that only **one** query is executed against the database.
...
- Use nested reads ([`include`](https://docs.prisma.io/docs/orm/reference/prisma-client-reference#include) ) to return users and related posts
- Use the [`in`](https://docs.prisma.io/docs/orm/reference/prisma-client-reference#in) filter
- Set the [`relationLoadStrategy`](https://docs.prisma.io/docs/orm/prisma-client/queries/relation-queries#relation-load-strategies-preview) to `"join"`

#

- Solving n+1 with `include`

You can use `include` to return each user's posts. This only results in **two** SQL queries - one to get users, and one to get posts. This is known as a [nested read](https://docs.prisma.io/docs/orm/prisma-client/queries/relation-queries#nested-reads).
