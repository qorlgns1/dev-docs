...
Source URL: https://docs.prisma.io/docs/orm/prisma-client/deployment/edge/deploy-to-vercel
...
You can use Prisma Postgres in Vercel's edge runtime. Follow this guide for an end-to-end tutorial on [deploying an application to Vercel using Prisma Postgres](https://docs.prisma.io/docs/guides/frameworks/nextjs).
...
If your application uses PostgreSQL, we recommend using [Prisma Postgres](https://docs.prisma.io/docs/postgres). It is fully supported on edge runtimes and does not require a specialized edge-compatible driver. For other databases, [Prisma Accelerate](https://docs.prisma.io/docs/accelerate) extends edge compatibility so you can connect to _any_ database from _any_ edge function provider.
...
When in **development** , you can configure your database connection via the `DATABASE_URL` environment variable (e.g. [using `.env` files](https://docs.prisma.io/docs/orm/more/dev-environment/environment-variables)).
...
If you don't have a project to deploy, follow the instructions in the [Prerequisites](https://docs.prisma.io/docs/orm/prisma-client/deployment/edge/deploy-to-vercel#prerequisites) to bootstrap a basic Next.js app with Prisma ORM in it.
...
Use `attachDatabasePool` together with [Prisma's driver adapters](https://docs.prisma.io/docs/orm/core-concepts/supported-databases/database-drivers) to safely manage connections in Fluid:
