...
Source URL: https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries

# Raw queries

Learn how you can send raw SQL and MongoDB queries to your database using the raw() methods from the Prisma Client API

We recommend using [TypedSQL](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql) for type-safe SQL queries instead of the raw queries described below.
...
You can also use the [`Prisma.sql`](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#tagged-template-helpers) helper, in fact, the `$queryRaw` method will **only accept** a template string or the `Prisma.sql` helper:
...
- Prisma maps any database values returned by `$queryRaw` and `$queryRawUnsafe` to their corresponding JavaScript types. [Learn more](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#raw-query-type-mapping).

- `$queryRaw` does not support dynamic table names in PostgreSQL databases. [Learn more](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#dynamic-table-names-in-postgresql)
...
You can also [type the results of `$queryRaw`](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#typing-queryraw-results).
...
If you are selecting **specific fields** of the model or want to include relations, refer to the documentation about [leveraging Prisma Client's generated types](https://docs.prisma.io/docs/orm/prisma-client/type-safety/operating-against-partial-structures-of-model-types#problem-using-variations-of-the-generated-model-type) if you want to make sure that the results are properly typed.
...
[It is not possible to interpolate table names](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#considerations). This means that you cannot use dynamic table names with `$queryRaw`. Instead, you must use [`$queryRawUnsafe`](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#queryrawunsafe), as follows:
...
Note that if you use `$queryRawUnsafe` in conjunction with user inputs, you risk SQL injection attacks. [Learn more](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#queryrawunsafe).
...
Wherever possible you should use the `$queryRaw` method instead. When used correctly `$queryRaw` method is significantly safer but note that the `$queryRaw` method can also be made vulnerable in certain circumstances. For more information, see the [SQL injection prevention](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#sql-injection-prevention) section below.
...
For more details on using parameterized queries, see the [parameterized queries](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#parameterized-queries) section below.
...
- [`PREPARE` does not support `ALTER`](https://www.postgresql.org/docs/current/sql-prepare.html) \- see the [workaround](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#alter-limitation-postgresql).
...
Wherever possible you should use the `$executeRaw` method instead. When used correctly `$executeRaw` method is significantly safer but note that the `$executeRaw` method can also be made vulnerable in certain circumstances. For more information, see the [SQL injection prevention](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#sql-injection-prevention) section below.
...
You can use `.$executeRaw()` and `.$queryRaw()` inside a [transaction](https://docs.prisma.io/docs/orm/prisma-client/queries/transactions).
...
- [SQL Template Tag](https://github.com/blakeembrey/sql-template-tag) give you access to [useful helpers](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#tagged-template-helpers).
...
[`Unsupported` types](https://docs.prisma.io/docs/orm/reference/prisma-schema-reference#unsupported) need to be cast to Prisma Client supported types before using them in `$queryRaw` or `$queryRawUnsafe`. For example, take the following model, which has a `location` field with an `Unsupported` type:
...
For details of supported Prisma types, see the [Prisma connector overview](https://docs.prisma.io/docs/orm/core-concepts/supported-databases) for the relevant database.
...
If you cannot use tagged templates, you can instead use [`$queryRawUnsafe`](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#queryrawunsafe) or [`$executeRawUnsafe`](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#executerawunsafe). However, **be aware that these functions significantly increase the risk of SQL injection vulnerabilities in your code**.
...
- `find` (use [`findRaw()`](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#findraw) instead)
- `aggregate` (use [`aggregateRaw()`](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#aggregateraw) instead)
...
Do not use `$runCommandRaw()` for queries which contain the `"find"` or `"aggregate"` commands, because you might be unable to fetch all data. This is because MongoDB returns a [cursor](https://www.mongodb.com/docs/manual/tutorial/iterate-a-cursor/) that is attached to your MongoDB session, and you might not hit the same MongoDB session every time. For these queries, you should use the specialised [`findRaw()`](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#findraw) and [`aggregateRaw()`](https://docs.prisma.io/docs/orm/prisma-client/using-raw-sql/raw-queries#aggregateraw) methods instead.
