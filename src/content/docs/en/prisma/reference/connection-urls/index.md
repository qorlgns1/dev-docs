...
Source URL: https://docs.prisma.io/docs/orm/reference/connection-urls

# Connection URLs

Learn about the format and syntax Prisma ORM uses for defining database connection URLs for PostgreSQL, MySQL and SQLite

Prisma ORM needs a connection URL to be able to connect to your database, e.g. when sending queries with [Prisma Client](https://docs.prisma.io/docs/orm/prisma-client/setup-and-configuration/introduction) or when changing the database schema with [Prisma Migrate](https://docs.prisma.io/docs/orm/prisma-migrate).

The connection URL is provided via the `url` field of a `datasource` block in your Prisma config (or Prisma schema if on version 6). It usually consists of the following components (except for SQLite and [Prisma Postgres](https://docs.prisma.io/docs/postgres)):
...
Make sure you have this information at hand when getting started with Prisma ORM. If you don't have a database server running yet, you can either use a local SQLite database file (see the [Quickstart](https://docs.prisma.io/docs/prisma-orm/quickstart/sqlite)) or [setup a free PostgreSQL database with Prisma Postgres](https://docs.prisma.io/docs/postgres).
...
For Microsoft SQL Server, you must [escape special characters](https://docs.prisma.io/docs/orm/core-concepts/supported-databases/sql-server#connection-details) in any part of your connection string.
...
[Prisma Postgres](https://docs.prisma.io/docs/postgres) is a managed PostgreSQL service running on unikernels. There are several ways to connect to Prisma Postgres:

- via direct TCP connections (lets you connect via any ORM or database tool)
- via [Prisma Accelerate](https://docs.prisma.io/docs/accelerate) (only supported with Prisma ORM)
- locally
...
The connection string for connecting to a [local Prisma Postgres](https://docs.prisma.io/docs/postgres/database/local-development) instance mirrors the structure of a remote instance via Accelerate:
...
However, in this case the `API_KEY` doesn't provide authentication details. Instead, it encodes information about the local Prisma Postgres instance. You can obtain a local connection string via the [`prisma dev`](https://docs.prisma.io/docs/orm/reference/prisma-cli-reference#dev) command.
...
_Support for MongoDB is limited to[Prisma 6](https://docs.prisma.io/docs/v6/orm/reference/connection-urls#mongodb) as of now. We're working on support for MongoDB in Prisma v7_
