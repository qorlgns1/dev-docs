...
When you’re ready, follow one of the paths [below](https://developers.openai.com/codex/app/worktrees#verifying-and-pushing-workflow-changes) based on your project and flow.
...
Because of this, choose how you want to verify and commit changes Codex made on a worktree:
  ...
  2. [Work in your local checkout](https://developers.openai.com/codex/app/worktrees#option-2-working-in-your-local-checkout). Use this when you need to bring changes back into your main checkout, for example because you can run only one instance of your app.
...
If you plan to keep working on this branch, you can [add it to the sidebar](https://developers.openai.com/codex/app/worktrees#adding-a-worktree-to-the-sidebar). Otherwise, archive the thread after you’re done so the worktree can be deleted.
...
If you plan on checking out the branch locally, try Workflow 2 ([sync with local](https://developers.openai.com/codex/app/worktrees#option-2-working-in-your-local-checkout)).
...
Not yet. If you need to change environments, you have to start a new thread in the target environment and restate the prompt. You can use the up arrow keys in the composer to try to recover your prompt.

What happens to threads if a worktree is deleted?

Threads can remain in your history even if the underlying worktree directory is cleaned up. However, Codex saves a snapshot of the worktree prior to cleaning it up and offers to restore it if you reopen the thread associated with it.