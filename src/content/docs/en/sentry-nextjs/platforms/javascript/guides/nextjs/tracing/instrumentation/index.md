---
title: 'Instrumentation | Sentry for Next.js'
description: "To capture transactions and spans customized to your organization's needs, you must first set up tracing."
---

Source URL: https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation

# Instrumentation | Sentry for Next.js

To capture transactions and spans customized to your organization's needs, you must first [set up tracing.](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing.md)
...
You can find a list of all tracing APIs in the [Tracing API](https://docs.sentry.io/platforms/javascript/guides/nextjs/apis.md#tracing) section.
...
* [startSpan](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#starting-an-active-span-startspan): Creates a new span that is active, and which will end automatically. You'll likely want to use this function.
* [startSpanManual](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#starting-an-active-span-with-manual-end-startspanmanual): Creates a new span that is active, which has to be ended manually.
* [startInactiveSpan](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#starting-inactive-spans-startinactivespan): Creates a new span that is inactive, which has to be ended manually.

## [Active vs. Inactive Spans](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#active-vs-inactive-spans)
...
In places where you are not able to execute your code in a callback (for example, when working with hooks or similar) you have to work with inactive spans, and can combine this with [withActiveSpan](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#withactivespan) to manually associate child spans with the correct parent span.

## [Span Hierarchy in the Browser](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#span-hierarchy-in-the-browser)
...
## [Span Starting Options](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#span-starting-options)
...
## [Starting an Active Span (`startSpan`)](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#starting-an-active-span-startspan)
...
## [Starting an Active Span with Manual End (`startSpanManual`)](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#starting-an-active-span-with-manual-end-startspanmanual)
...
## [Starting Inactive Spans (`startInactiveSpan`)](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#starting-inactive-spans-startinactivespan)

To add spans that aren't active, you can create independent spans. This is useful when you have work that is grouped together under a single parent span, but is independent from the currently active span. However, in most cases you'll want to create and use the [startSpan](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#starting-an-active-span-startspan) API from above.
...
- [Setting an inactive span active (browser only)](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#setting-an-inactive-span-active-browser-only)

Available since: `v10.15.0`

In browser environments, you might run into use cases, where the callback-based span APIs are not sufficient. In such cases (see example below), you can use `startInactiveSpan` to start an initially inactive span and then set it active until you manually end the span.
...
Using `startInactiveSpan` in combination with `setActiveSpanInBrowser` allows you to create an inactive span that can be made active until you manually end it. In contrast, [`startSpanManual`](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#starting-an-active-span-with-manual-end-startspanmanual) allows you to end the span manually at any point, but it only remains active within the callback.

Note that `setActiveSpanInBrowser` is only available in browser environments! If you're running code in the server (for example for server-side rendering), make sure to guard this call to only run in the browser.

## [Starting Spans as Children of a Specific Span](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#starting-spans-as-children-of-a-specific-span)
...
## [Utilities To Work With Spans](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#utilities-to-work-with-spans)

We expose some helpful utilities that can help you with custom instrumentation. See [Tracing Utility APIs](https://docs.sentry.io/platforms/javascript/guides/nextjs/apis.md#tracing-utilities) for more information.

## [Distributed Tracing](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#distributed-tracing)

See [Custom Trace Propagation](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/trace-propagation/custom-instrumentation.md) for details on how to manually set up distributed tracing.

## [Improving Span Data](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#improving-span-data)

- [Adding Span Attributes](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#adding-span-attributes)
...
- [Adding attributes to all spans](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#adding-attributes-to-all-spans)
...
- [Adding Span Operations ("op")](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#adding-span-operations-op)
...
- [Updating the Span Name](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#updating-the-span-name)
...
- [Updating the Span Status](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation.md#updating-the-span-status)
...
Alternatively, you can also use the [`Sentry.setHttpStatus()`](https://docs.sentry.io/platforms/javascript/guides/nextjs/apis.md#setHttpStatus) utility function to set a more specific error status.

## Pages in this section

- [Automatic Instrumentation](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation/automatic-instrumentation.md)
- [Instrument Queues](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation/queues-module.md)
- [Instrument MCP Servers](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation/mcp-module.md)
- [Instrument Caches](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation/caches-module.md)
- [Instrument HTTP Requests](https://docs.sentry.io/platforms/javascript/guides/nextjs/tracing/instrumentation/requests-module.md)
