...
description: 'In Zod, validation errors are surfaced as instances of the  class.'
---

Source URL: https://zod.dev/error-customization
...
Return `undefined` to avoid customizing the error message and fall back to the default message. (More specifically, Zod will yield control to the next error map in the [precedence chain](https://zod.dev/error-customization#error-precedence).) This is useful for selectively customizing certain error messages but not others.
```
    z.int64({
      error: (issue) => {
