---
title: 'Setup locale-based routing'
description: 'Prefer to watch a video?'
---

Source URL: https://next-intl.dev/docs/routing/setup
...
## Initial setup[](https://next-intl.dev/docs/routing/setup#initial-setup)
...
[](https://next-intl.dev/docs/routing/setup#proxy-matcher-dots)How can I match pathnames that contain dots like `/users/jane.doe`?
...
In case you ran into an issue, have a look at the [App Router example](https://next-intl.dev/examples#app-router) to explore a working app.

## Static rendering[](https://next-intl.dev/docs/routing/setup#static-rendering)
...
### Add `generateStaticParams`[](https://next-intl.dev/docs/routing/setup#add-generatestaticparams)
...
[](https://next-intl.dev/docs/routing/setup#generatestaticparams-locales)Do I need to return all locales from `generateStaticParams`?
...
### Add `setRequestLocale` to all relevant layouts and pages[](https://next-intl.dev/docs/routing/setup#add-setrequestlocale-to-all-relevant-layouts-and-pages)
...
**Keep in mind that:**
  ...
  1. The locale that you pass to `setRequestLocale` should be validated (e.g. in your [root layout](https://next-intl.dev/docs/routing/setup#layout)).
...
[](https://next-intl.dev/docs/routing/setup#setrequestlocale-implementation)How does setRequestLocale work?
...
[](https://next-intl.dev/docs/routing/setup#setrequestlocale-background)Why is this API necessary?
...
### Use the `locale` param in metadata[](https://next-intl.dev/docs/routing/setup#use-the-locale-param-in-metadata)

In addition to the rendering of your pages, also page metadata needs to qualify for static rendering.

To achieve this, you can forward the `locale` that you receive from Next.js via `params` to the [awaitable functions from `next-intl`](https://next-intl.dev/docs/environments/server-client-components#async-components).
