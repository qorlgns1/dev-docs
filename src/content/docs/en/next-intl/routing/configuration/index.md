...
description: 'The routing configuration that is shared between the middleware and the navigation APIs can be defined with the  function.'
---

Source URL: https://next-intl.dev/docs/routing/configuration

# Routing configuration

## `defineRouting`[](https://next-intl.dev/docs/routing/configuration#definerouting)

The routing configuration that is shared between the [middleware](https://next-intl.dev/docs/routing/middleware) and the [navigation APIs](https://next-intl.dev/docs/routing/navigation) can be defined with the `defineRouting` function.
...
[](https://next-intl.dev/docs/routing/configuration#locales-unknown)What if the locales aren’t known at build time?

In case you’re building an app where locales can be added and removed at runtime, you can provide the routing configuration for the middleware [dynamically per request](https://next-intl.dev/docs/routing/middleware#composing-other-middlewares).

To create the corresponding navigation APIs, you can [omit the `locales` argument](https://next-intl.dev/docs/routing/navigation#locales-unknown) from `createNavigation` in this case.
...
#### `localePrefix: 'always'` (default)[](https://next-intl.dev/docs/routing/configuration#locale-prefix-always)
...
**Note that:**
  ...
  1. If you use this routing strategy, make sure that your [`matcher`](https://next-intl.dev/docs/routing/middleware#matcher-config) detects unprefixed pathnames.
  2. The middleware will by default set a [cookie](https://next-intl.dev/docs/routing/configuration#locale-cookie) to remember the user’s locale preference. If no explicit locale prefix is present in the pathname, then [locale detection](https://next-intl.dev/docs/routing/middleware#locale-detection-prefix) will potentially redirect users to the latest locale that was matched based on the cookie value (e.g. `/` → `/de`).
  3. If a superfluous locale prefix like `/en/about` is requested, the middleware will automatically redirect to the unprefixed version `/about`. This can be helpful in case you’re redirecting from another locale and you want to update a potential cookie value first (e.g. [`<Link />`](https://next-intl.dev/docs/routing/navigation#link) relies on this mechanism).
...
However, you can also configure the middleware to never show a locale prefix in the URL, which can be helpful in the following cases:
  ...
  1. You want to use [domain-based routing](https://next-intl.dev/docs/routing/configuration#domains) and there’s only one locale per domain
...
**Note that:**
  ...
  1. If you use this routing strategy, make sure that your [`matcher`](https://next-intl.dev/docs/routing/middleware#matcher-config) detects unprefixed pathnames.
  2. [Alternate links](https://next-intl.dev/docs/routing/configuration#alternate-links) are disabled in this mode since URLs might not be unique per locale. Due to this, consider including these yourself, or set up a [sitemap](https://next-intl.dev/docs/environments/actions-metadata-route-handlers#sitemap) that links localized pages via `alternates`.
  3. You can consider increasing the [`maxAge`](https://next-intl.dev/docs/routing/configuration#locale-cookie) attribute of the locale cookie to a longer duration to remember the user’s preference across sessions.
...
**Note that:**
  ...
  1. You should adapt your [`matcher`](https://next-intl.dev/docs/routing/middleware#matcher-config) to match the custom prefixes.
...
[](https://next-intl.dev/docs/routing/configuration#locale-prefix-custom-read-prefix)Can I read the matched prefix in my app?
...
[](https://next-intl.dev/docs/routing/configuration#localized-pathnames-dynamic-segments)How can I localize CMS-driven URLs?
...
[](https://next-intl.dev/docs/routing/configuration#localized-pathnames-revalidation)How can I revalidate localized pathnames?
...
In many cases, `domains` are combined with a [`localePrefix`](https://next-intl.dev/docs/routing/configuration#locale-prefix) setting to achieve results as shown above. Also [custom prefixes](https://next-intl.dev/docs/routing/configuration#locale-prefix-prefixes) can be used to customize the user-facing prefix per locale.
...
Locales are required to be unique across domains, therefore regional variants are typically used to avoid conflicts. Note however that you don’t necessarily need to [provide messages](https://next-intl.dev/docs/usage/configuration#messages) for each locale if the overall language is sufficient for your use case.

If no domain matches, the middleware will fall back to default locale matching based on [prefixes](https://next-intl.dev/docs/routing/middleware#location-detection-prefix) (e.g. on `localhost`, for local development).

See also: [Domain-based routing](https://next-intl.dev/docs/routing/middleware#location-detection-domain)

[](https://next-intl.dev/docs/routing/configuration#domains-localeprefix-individual)Can I use a different `localePrefix` setting per domain?
...
The middleware will [detect a matching locale](https://next-intl.dev/docs/routing/middleware#locale-detection) based on your routing configuration & the incoming request and will either pass the request through for a matching locale or redirect to one that matches.
...
In this case, only the locale prefix and a potentially [matching domain](https://next-intl.dev/docs/routing/configuration#domains) are used to determine the locale.

### `localeCookie`[](https://next-intl.dev/docs/routing/configuration#locale-cookie)

If a user changes the locale to a value that doesn’t match the `accept-language` header, `next-intl` will set a session cookie called `NEXT_LOCALE` that contains the most recently detected locale. This is used to [remember the user’s locale](https://next-intl.dev/docs/routing/middleware#locale-detection) preference for subsequent requests.
...
By default, the cookie will be configured with the following attributes:
  ...
  3. [**`path`**](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Set-Cookie#pathpath-value): This value is not set by default, but will use the value of your [`basePath`](https://next-intl.dev/docs/routing/configuration#basepath) if configured.
...
[](https://next-intl.dev/docs/routing/configuration#locale-cookie-gdpr)Which `maxAge` value should I consider for GDPR compliance?
...
[](https://next-intl.dev/docs/routing/configuration#alternate-links-details)Which alternate links are included?
...
[](https://next-intl.dev/docs/routing/configuration#alternate-links-customization)Can I customize the alternate links?

If you need to customize the alternate links, you can either turn them off and provide your own implementation, or if you only need to make minor adaptions, you can [compose the middleware](https://next-intl.dev/docs/routing/configuration#composing-other-middlewares) and add your custom logic after the middleware has run:
...
The only exception is the [`getPathname`](https://next-intl.dev/docs/routing/navigation#getpathname) function, which will return the bare pathname without a base path. Due to this, you can prefix the returned pathname manually if you need to.

When using a base path, you should make sure that your [`matcher`](https://next-intl.dev/docs/routing/configuration#matcher-config) handles an explicit root:
...
Note that if you’re using [`pathnames`](https://next-intl.dev/docs/routing/configuration#pathnames), your internal and external pathnames can be defined either with or without a trailing slash as they will be normalized internally.
