---
title: 'Rendering translations'
description: 'Prefer to watch a video?'
---

Source URL: https://next-intl.dev/docs/usage/translations

# Rendering translations

Prefer to watch a video?

[First translations](https://learn.next-intl.dev/chapters/03-translations/01-setup)

## Terminology[](https://next-intl.dev/docs/usage/translations#terminology)
...
## Structuring messages[](https://next-intl.dev/docs/usage/translations#structuring-messages)
...
[Organizing keys](https://learn.next-intl.dev/chapters/03-translations/03-keys)

[Reusing messages](https://learn.next-intl.dev/chapters/03-translations/04-message-reuse)

[Editor tools](https://learn.next-intl.dev/chapters/03-translations/05-tooling)

💡

Translators can collaborate on messages by using a localization management solution like [Crowdin](https://crowdin.com/teams/engineering).

[](https://next-intl.dev/docs/usage/translations#messages-nested-structure)How can I provide more structure for messages?
...
[](https://next-intl.dev/docs/usage/translations#messages-outside-components)How can I use translations outside of components?
...
There’s one exception however: [Using `next-intl` with Server Actions, Metadata and Route Handlers](https://next-intl.dev/docs/environments/actions-metadata-route-handlers).

If you’re interested to dive deeper into this topic, there’s a blog post that discusses the background of this design decision: [How (not) to use translations outside of React components](https://next-intl.dev/blog/translations-outside-of-react-components).

[](https://next-intl.dev/docs/usage/translations#messages-other-styles)Can I use a different style for structuring my messages?
...
[ICU messages](https://learn.next-intl.dev/chapters/03-translations/06-icu-messages)

### Static messages[](https://next-intl.dev/docs/usage/translations#static-messages)
...
### Interpolation of arguments[](https://next-intl.dev/docs/usage/translations#arguments)
...
[](https://next-intl.dev/docs/usage/translations#interpolation-supported-characters)Which characters are supported for value names?
...
### Cardinal pluralization[](https://next-intl.dev/docs/usage/translations#cardinal-pluralization)
...
Note that by using the `#` marker, the value will be [formatted as a number](https://next-intl.dev/docs/usage/numbers).

[](https://next-intl.dev/docs/usage/translations#cardinal-plural-forms)Which plural forms are supported?
...
### Ordinal pluralization[](https://next-intl.dev/docs/usage/translations#ordinal-pluralization)
...
[](https://next-intl.dev/docs/usage/translations#ordinal-plural-forms)Which plural forms are supported?
...
### Selecting enum-based values[](https://next-intl.dev/docs/usage/translations#selecting-enum-based-values)
...
[](https://next-intl.dev/docs/usage/translations#select-values-supported-characters)Which characters are supported for select values?
...
### Escaping[](https://next-intl.dev/docs/usage/translations#escaping)

Since curly braces are used for [interpolating arguments](https://next-intl.dev/docs/usage/translations#arguments), you can wrap them in single quotes (`'`) to use the actual symbol in messages:
...
## Rich text[](https://next-intl.dev/docs/usage/translations#rich-text)
...
[](https://next-intl.dev/docs/usage/translations#rich-text-reuse-tags)How can I reuse tags across my app?
...
[](https://next-intl.dev/docs/usage/translations#rich-text-self-closing)How can I use “self-closing” tags without any chunks?
...
[](https://next-intl.dev/docs/usage/translations#rich-text-attributes)How can I pass attributes to tags?
...
For the use case of localizing pathnames, consider using [`pathnames`](https://next-intl.dev/docs/routing/configuration#pathnames).

## HTML markup[](https://next-intl.dev/docs/usage/translations#html-markup)

To render rich text, you typically want to use [rich text formatting](https://next-intl.dev/docs/usage/translations#rich-text). However, if you have a use case where you need to emit raw HTML markup, you can use the `t.markup` function:
...
## Raw messages[](https://next-intl.dev/docs/usage/translations#raw-messages)
...
**Important** : You should always sanitize the content that you pass to [`dangerouslySetInnerHTML`](https://react.dev/reference/react-dom/components/common#dangerously-setting-the-inner-html) to avoid cross-site scripting attacks.

The value of a raw message can be any valid JSON value: strings, booleans, objects and arrays.

## Optional messages[](https://next-intl.dev/docs/usage/translations#t-has)
...
Note that separately from this, you can also provide [fallback messages](https://next-intl.dev/docs/usage/configuration#messages), e.g. from the default locale, in case you have incomplete messages for certain locales.

## Arrays of messages[](https://next-intl.dev/docs/usage/translations#arrays-of-messages)
...
This approach ensures that you can use ICU features while also enabling static [validation of messages](https://next-intl.dev/docs/workflows/messages).

[](https://next-intl.dev/docs/usage/translations#arrays-varying-amount)What if the amount of items varies depending on the locale?

To dynamically iterate over all keys of a namespace, you can use the [`useMessages`](https://next-intl.dev/docs/usage/configuration#messages) hook to retrieve all messages of a given namespace and extract the keys from there:
...
## Right-to-left languages[](https://next-intl.dev/docs/usage/translations#right-to-left-languages)
