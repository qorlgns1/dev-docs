...
description: 'Placeholder data allows a query to behave as if it already has data, similar to the  option, but the data is not persisted to the cache. This comes in...'
---

Source URL: https://tanstack.com/query/latest/docs/framework/react/guides/placeholder-query-data
...
- Imperatively:
  - [Prefetch or fetch the data using `queryClient` and the `placeholderData` option](https://tanstack.com/query/latest/docs/framework/react/guides/prefetching.md)
...
`placeholderData` can also be a function, where you can get access to the data and Query meta information of a "previous" successful Query. This is useful for situations where you want to use the data from one query as the placeholder data for another query. When the QueryKey changes, e.g. from `['todos', 1]` to `['todos', 2]`, we can keep displaying "old" data instead of having to show a loading spinner while data is _transitioning_ from one Query to the next. For more information, see [Paginated Queries](https://tanstack.com/query/latest/docs/framework/react/guides/paginated-queries.md).
