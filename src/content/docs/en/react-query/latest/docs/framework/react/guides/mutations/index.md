---
title: 'Mutations'
description: 'Unlike queries, mutations are typically used to create/update/delete data or perform server side-effects. For this purpose, TanStack Query exports a  ...'
---

Source URL: https://tanstack.com/query/latest/docs/framework/react/guides/mutations
...
Even with just variables, mutations aren't all that special, but when used with the `onSuccess` option, the [Query Client's `invalidateQueries` method](https://tanstack.com/query/latest/docs/reference/QueryClient.md#queryclientinvalidatequeries) and the [Query Client's `setQueryData` method](https://tanstack.com/query/latest/docs/reference/QueryClient.md#queryclientsetquerydata), mutations become a very powerful tool.
...
If you persist offline mutations with the [persistQueryClient plugin](https://tanstack.com/query/latest/docs/framework/react/plugins/persistQueryClient.md), mutations cannot be resumed when the page is reloaded unless you provide a default mutation function.
