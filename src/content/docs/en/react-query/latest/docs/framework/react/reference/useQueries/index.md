---
title: 'useQueries'
description: 'The  hook can be used to fetch a variable number of queries:'
---

Source URL: https://tanstack.com/query/latest/docs/framework/react/reference/useQueries
...
The `useQueries` hook accepts an options object with a **queries** key whose value is an array with query option objects identical to the [`useQuery` hook](https://tanstack.com/query/latest/docs/framework/react/reference/useQuery.md) (excluding the `queryClient` option - because the `QueryClient` can be passed in on the top level).
