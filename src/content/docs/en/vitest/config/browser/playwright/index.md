...
description: "To run tests using playwright, you need to install the  npm package and specify its  export in the  property of your config:"
---

Source URL: https://vitest.dev/config/browser/playwright
...
::: warning
Vitest will ignore `launch.headless` option. Instead, use [`test.browser.headless`](https://vitest.dev/config/browser/headless).

Note that Vitest will push debugging flags to `launch.args` if [`--inspect`](https://vitest.dev/guide/cli#inspect) is enabled.
:::
...
It is also recommended to use [`test.browser.viewport`](https://vitest.dev/config/browser/headless) instead of specifying it here as it will be lost when tests are running in headless mode.
:::
...
This value configures the default timeout it takes for Playwright to wait until all accessibility checks pass and [the action](https://vitest.dev/api/browser/interactivity) is actually done.
