---
title: "onStackTrace"
description: "Apply a filtering function to each frame of each stack trace when handling errors. This does not apply to stack traces printed by . The first argument..."
---

Source URL: https://vitest.dev/config/onstacktrace

# onStackTrace

- **Type**: `(error: Error, frame: ParsedStack) => boolean | void`

Apply a filtering function to each frame of each stack trace when handling errors. This does not apply to stack traces printed by [`printConsoleTrace`](https://vitest.dev/config/printconsoletrace#printconsoletrace). The first argument, `error`, is a `TestError`.
