---
title: "server"
description: "Before Vitest 4, this option was used to define the configuration for the  server."
---

Source URL: https://vitest.dev/config/server
...
- **Type:** `(string | RegExp)[]`
- **Default:** files inside [`moduleDirectories`](https://vitest.dev/config/deps#moduledirectories)
...
::: tip
If a string is provided, it is first normalized by prefixing the `/node_modules/` or other [`moduleDirectories`](https://vitest.dev/config/deps#moduledirectories) segments (for example, `'react'` becomes `/node_modules/react/`), and the resulting string is then matched against the full file path. For example, package `@company/some-name` located inside `packages/some-name` should be specified as `some-name`, and `packages` should be included in `deps.moduleDirectories`.
