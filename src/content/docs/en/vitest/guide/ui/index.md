---
title: "Vitest UI"
description: "Powered by Vite, Vitest also has a dev server under the hood when running the tests. This allows Vitest to provide a beautiful UI to view and interact..."
---

Source URL: https://vitest.dev/guide/ui
...
This view is separated into two parts. The top part shows the full module ID and some diagnostics about the module. If [`experimental.fsModuleCache`](https://vitest.dev/config/experimental#experimental-fsmodulecache) is enabled, there will be a "cached" or "not cached" badge. On the right you can see time diagnostics:
...
::: tip
If you are developing a custom integration on top of Vitest, you can use [`vitest.experimental_getSourceModuleDiagnostic`](https://vitest.dev/api/advanced/vitest#getsourcemodulediagnostic) to retrieve this information.
:::
...
By default, Vitest shows the breakdown automatically if there is at least one module that took longer than 500 milliseconds to load. You can control the behaviour by setting the [`experimental.printImportBreakdown`](https://vitest.dev/config/experimental#experimental-printimportbreakdown) option.
