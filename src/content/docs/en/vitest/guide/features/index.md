---
title: "Features"
description: "Vite's config, transformers, resolvers, and plugins. Use the same setup from your app to run the tests."
---

Source URL: https://vitest.dev/guide/features
...
By default Vitest runs test files in [multiple processes](https://vitest.dev/guide/parallelism) using [`node:child_process`](https://nodejs.org/api/child_process.html), allowing tests to run simultaneously. If you want to speed up your test suite even further, consider enabling `--pool=threads` to run tests using [`node:worker_threads`](https://nodejs.org/api/worker_threads.html) (beware that some packages might not work with this setup).
To run tests in a single thread or process, see [`fileParallelism`](https://vitest.dev/config/#fileParallelism).
...
Run tests on different machines using [`--shard`](https://vitest.dev/guide/cli#shard) and [`--reporter=blob`](https://vitest.dev/guide/reporters#blob-reporter) flags.
