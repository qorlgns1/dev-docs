---
title: "Trace View"
description: "Vitest Browser Mode supports generating Playwright's trace files. To enable tracing, you need to set the  option in the  configuration."
---

Source URL: https://vitest.dev/guide/browser/trace-view

# Trace View

Vitest Browser Mode supports generating Playwright's [trace files](https://playwright.dev/docs/trace-viewer#viewing-remote-traces). To enable tracing, you need to set the [`trace`](https://vitest.dev/config/browser/trace) option in the `test.browser` configuration.

::: warning
Generating trace files is only available when using the [Playwright provider](https://vitest.dev/config/browser/playwright).
:::
...
The traces are available in reporters as [annotations](https://vitest.dev/guide/test-annotations). For example, in the HTML reporter, you can find the link to the trace file in the test details.
