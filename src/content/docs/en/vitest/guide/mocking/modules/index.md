...
description: 'Before mocking a "module", we should define what it is. In Vitest context, the "module" is a file that exports something. Using plugins, any file can ...'
---

Source URL: https://vitest.dev/guide/mocking/modules
...
To mock a module completely, you can use the [`vi.mock` API](https://vitest.dev/api/vi#vi-mock). You can define a new module dynamically by providing a factory that returns a new module as a second argument:
...
::: tip
Remember that you can call `vi.mock` in a [setup file](https://vitest.dev/config/setupfiles) to apply the module mock in every test file automatically.
:::
...
This will not work in the [Browser Mode](https://vitest.dev/guide/browser/) because it uses the browser's native ESM support to serve modules. The module namespace object is sealed and can't be reconfigured. To bypass this limitation, Vitest supports `{ spy: true }` option in `vi.mock('./example.js')`. This will automatically spy on every export in the module without replacing them with fake ones.
...
To redirect the import, use [`test.alias`](https://vitest.dev/config/#alias) config option:
