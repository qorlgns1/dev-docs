...
description: "Understanding the test run lifecycle is essential for writing effective tests, debugging issues, and optimizing your test suite. This guide explains w..."
---

Source URL: https://vitest.dev/guide/lifecycle
...
1. **Global Setup** - One-time setup before any tests run
1. **Worker Creation** - Test workers are spawned based on the [pool](https://vitest.dev/config/pool) configuration
1. **Test File Collection** - Test files are discovered and organized
1. **Test Execution** - Tests run with their hooks and assertions
1. **Reporting** - Results are collected and reported
1. **Global Teardown** - Final cleanup after all tests complete

Phases 4–6 run once for each test file, so across your test suite they will execute multiple times and may also run in parallel across different files when you use more than [1 worker](https://vitest.dev/config/maxworkers).
...
- [Command-line](https://vitest.dev/guide/cli) arguments are parsed
- [Configuration file](https://vitest.dev/config/) is loaded
...
If you have configured [`globalSetup`](https://vitest.dev/config/globalsetup) files, they run once before any test workers are created.
...
- Tests cannot access variables defined in global setup (use [`provide`/`inject`](https://vitest.dev/config/provide) instead)
...
After global setup completes, Vitest creates test workers based on your [pool configuration](https://vitest.dev/config/pool).

**What happens:**

- Workers are spawned according to the `browser.enabled` or `pool` setting (`threads`, `forks`, `vmThreads`, or `vmForks`)
- Each worker gets its own isolated environment (unless [isolation](https://vitest.dev/config/isolate) is disabled)
- By default, workers are not reused to provide isolation. Workers are reused only if:
  - [isolation](https://vitest.dev/config/isolate) is disabled
...
Before each test file runs, [setup files](https://vitest.dev/config/setupfiles) are executed.

**What happens:**

- Setup files run in the same process as your tests
- By default, setup files run in **parallel** (configurable via [`sequence.setupFiles`](https://vitest.dev/config/sequence#sequence-setupfiles))
...
- If [isolation](https://vitest.dev/config/isolate) is disabled, setup files still rerun before each test file to trigger side effects, but imported modules are cached
...
- Files will run in **parallel** across different workers, configured by [`maxWorkers`](https://vitest.dev/config/maxworkers)
- Order can be randomized with [`sequence.shuffle`](https://vitest.dev/config/sequence#sequence-shuffle) or fine-tuned with [`sequence.sequencer`](https://vitest.dev/config/sequence#sequence-sequencer)
...
1. **Test collection** - `describe` blocks are processed, and tests are registered as side effects of importing the test file
1. **`beforeAll` hooks** - Run once before any tests in the suite
1. **For each test:**
   - `beforeEach` hooks execute (in order defined, or based on [`sequence.hooks`](https://vitest.dev/config/sequence#sequence-hooks))
   - Test function executes
   - `afterEach` hooks execute (reverse order by default with `sequence.hooks: 'stack'`)
   - [`onTestFinished`](https://vitest.dev/api/#ontestfinished) callbacks run (always in reverse order)
   - If test failed: [`onTestFailed`](https://vitest.dev/api/#ontestfailed) callbacks run
   - Note: if `repeats` or `retry` are set, all of these steps are executed again
1. **`afterAll` hooks** - Run once after all tests in the suite complete
...
When using `test.concurrent` or [`sequence.concurrent`](https://vitest.dev/config/sequence#sequence-concurrent):

- Tests within the same file can run in parallel
- Each concurrent test still runs its own `beforeEach` and `afterEach` hooks
- Use [test context](https://vitest.dev/guide/test-context) for concurrent snapshots: `test.concurrent('name', async ({ expect }) => {})`
...
For detailed information about the reporter lifecycle, see the [Reporters](https://vitest.dev/api/advanced/reporters) guide.
...
- `teardown()` functions from [`globalSetup`](https://vitest.dev/config/globalsetup) files run
...
1. **On file change:**
   - New [test run](https://vitest.dev/api/advanced/reporters#ontestrunstart) starts
   - Only affected test files are re-run
   - [Setup files](https://vitest.dev/config/setupfiles) run again for those test files
   - [Global setup](https://vitest.dev/config/globalsetup) does **not** re-run (use [`project.onTestsRerun`](https://vitest.dev/config/globalsetup#handling-test-reruns) for rerun-specific logic)
1. **On exit:**
...
- **Disabling [isolation](https://vitest.dev/config/isolate)** improves performance, but setup files still execute before each file
- **[Pool configuration](https://vitest.dev/config/pool)** affects parallelization and available APIs

For tips on how to improve performance, read the [Improving Performance](https://vitest.dev/guide/improving-performance) guide.
...
- Pool Configuration
- [Extending Reporters](https://vitest.dev/guide/advanced/reporters) - for reporter lifecycle events
- [Test API Reference](https://vitest.dev/api/) - for hook APIs and test functions
