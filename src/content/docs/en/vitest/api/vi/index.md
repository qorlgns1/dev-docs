---
title: "Mock Modules"
description: "Vitest provides utility functions to help you out through its  helper. You can access it globally (when globals configuration is enabled), or import i..."
---

Source URL: https://vitest.dev/api/vi
...
If there is no `__mocks__` folder or a factory provided, Vitest will import the original module and auto-mock all its exports. For the rules applied, see [algorithm](https://vitest.dev/guide/mocking/modules#automocking-algorithm).
...
Imports a module with all of its properties (including nested properties) mocked. Follows the same rules that [`vi.mock`](#vi-mock) does. For the rules applied, see [algorithm](https://vitest.dev/guide/mocking/modules#automocking-algorithm).
...
Creates a spy on a function, but can also be initiated without one. Every time a function is invoked, it stores its call arguments, returns, and instances. Additionally, you can manipulate its behavior with [methods](https://vitest.dev/api/mock).
...
Deeply mocks properties and methods of a given object in the same way as `vi.mock()` mocks module exports. See [automocking](https://vitest.dev/guide/mocking.html#automocking-algorithm) for the detail.
...
Calls [`.mockClear()`](https://vitest.dev/api/mock#mockclear) on all spies.
...
Calls [`.mockReset()`](https://vitest.dev/api/mock#mockreset) on all spies.
...
::: warning
This method also does not affect mocks created during [automocking](https://vitest.dev/guide/mocking/modules#mocking-a-module).

Note that unlike [`mock.mockRestore`](https://vitest.dev/api/mock#mockrestore), `vi.restoreAllMocks` will not clear mock history or reset the mock implementation
:::
...
::: tip
You can call [`vi.restoreAllMocks`](#vi-restoreallmocks) inside [`afterEach`](https://vitest.dev/api/#aftereach) (or enable [`test.restoreMocks`](https://vitest.dev/config/#restoreMocks)) to restore all methods to their original implementations after every test. This will restore the original [object descriptor](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/defineProperty), so you won't be able to change method's implementation anymore, unless you spy again:
...
This sections describes how to work with [fake timers](https://vitest.dev/guide/mocking/timers).
...
it will throw after 10 000 tries (can be configured with [`fakeTimers.loopLimit`](https://vitest.dev/config/#faketimers-looplimit)).
