---
title: "Plugin API 3.1.0 {#plugin-api}"
description: "This is an advanced API. If you just want to run tests, you probably don't need this. It is primarily used by library authors."
---

Source URL: https://vitest.dev/api/advanced/plugin

# Plugin API 3.1.0 {#plugin-api}

::: warning
This is an advanced API. If you just want to [run tests](https://vitest.dev/guide/), you probably don't need this. It is primarily used by library authors.
...
Unlike [`reporter.onInit`](https://vitest.dev/api/advanced/reporters#oninit), this hooks runs early in Vitest lifecycle allowing you to make changes to configuration like `coverage` and `reporters`. A more notable change is that you can manipulate the global config from a [test project](https://vitest.dev/guide/projects) if your plugin is defined in the project and not in the global config.

## Context

### project

The current [test project](https://vitest.dev/api/advanced/test-project) that the plugin belongs to.

::: warning Browser Mode
Note that if you are relying on a browser feature, the `project.browser` field is not set yet. Use [`reporter.onBrowserInit`](https://vitest.dev/api/advanced/reporters#onbrowserinit) event instead.
:::

### vitest

The global [Vitest](https://vitest.dev/api/advanced/vitest) instance. You can change the global configuration by directly mutating the `vitest.config` property:
...
This methods accepts a config glob pattern, a filepath to the config or an inline configuration. It returns an array of resolved [test projects](https://vitest.dev/api/advanced/test-project).
...
Vitest filters projects during the config resolution, so if the user defined a filter, injected project might not be resolved unless it [matches the filter](https://vitest.dev/api/advanced/vitest#matchesprojectfilter). You can update the filter via the `vitest.config.project` option to always include your test project:
...
This is called only if [`experimental.fsModuleCache`](https://vitest.dev/config/experimental#experimental-fsmodulecache) is defined.
