...
Source URL: https://vitest.dev/api/advanced/reporters
...
This guide lists all supported reporter methods. However, don't forget that instead of creating your own reporter, you can [extend existing one](https://vitest.dev/guide/advanced/reporters) instead:
...
This method is called when [Vitest](https://vitest.dev/api/advanced/vitest) was initiated or started, but before the tests were filtered.

::: info
Internally this method is called inside [`vitest.start`](https://vitest.dev/api/advanced/vitest#start), [`vitest.init`](https://vitest.dev/api/advanced/vitest#init) or [`vitest.mergeReports`](https://vitest.dev/api/advanced/vitest#mergereports). If you are using programmatic API, make sure to call either one depending on your needs before calling [`vitest.runTestSpecifications`](https://vitest.dev/api/advanced/vitest#runtestspecifications), for example. Built-in CLI will always run methods in correct order.
:::

Note that you can also get access to `vitest` instance from test cases, suites and test modules via a [`project`](https://vitest.dev/api/advanced/test-project) property, but it might also be useful to store a reference to `vitest` in this method.
...
This method is called when a new test run has started. It receives an array of [test specifications](https://vitest.dev/api/advanced/test-specification) scheduled to run. This array is readonly and available only for information purposes.
...
It receives a readonly list of test modules. You can iterate over it via a [`testModule.children`](https://vitest.dev/api/advanced/test-collection) property to report the state and errors, if any.
...
- `interrupted`: test was interrupted by [`vitest.cancelCurrentRun`](https://vitest.dev/api/advanced/vitest#cancelcurrentrun) call or `Ctrl+C` was pressed in the terminal (note that it's still possible to have failed tests in this case)

If Vitest didn't find any test files to run, this event will be invoked with empty arrays of modules and errors, and the state will depend on the value of [`config.passWithNoTests`](https://vitest.dev/config/#passwithnotests).
...
This method is called right before Vitest imports the setup file and the test module itself. This means that `testModule` will have no [`children`](https://vitest.dev/api/advanced/test-suite#children) yet, but you can start reporting it as the next test to run.
...
This method is called when all tests inside the file were collected, meaning [`testModule.children`](https://vitest.dev/api/advanced/test-suite#children) collection is populated, but tests don't have any results yet.
...
This method is called right after [`onTestModuleCollected`](#ontestmodulecollected) unless Vitest runs in collection mode ([`vitest.collect()`](https://vitest.dev/api/advanced/vitest#collect) or `vitest collect` in the CLI), in this case it will not be called at all because there are no tests to run.
...
This method is called when every test in the module finished running. This means, every test inside [`testModule.children`](https://vitest.dev/api/advanced/test-suite#children) will have a `test.result()` that is not equal to `pending`.
...
If `beforeAll` or `afterAll` are started, the `entity` will be either [`TestSuite`](https://vitest.dev/api/advanced/test-suite) or [`TestModule`](https://vitest.dev/api/advanced/test-module).

If `beforeEach` or `afterEach` are started, the `entity` will always be [`TestCase`](https://vitest.dev/api/advanced/test-case).
...
If `beforeAll` or `afterAll` have finished, the `entity` will be either [`TestSuite`](https://vitest.dev/api/advanced/test-suite) or [`TestModule`](https://vitest.dev/api/advanced/test-module).

If `beforeEach` or `afterEach` have finished, the `entity` will always be [`TestCase`](https://vitest.dev/api/advanced/test-case).
...
::: warning
Notice that it's possible to have [`testCase.result()`](https://vitest.dev/api/advanced/test-case#result) with `passed` or `failed` state already when `onTestCaseReady` is called. This can happen if test was running too fast and both `onTestCaseReady` and `onTestCaseResult` were scheduled to run in the same microtask.
:::
...
At this point, [`testCase.result()`](https://vitest.dev/api/advanced/test-case#result) will have non-pending state.
...
The `onTestAnnotate` hook is associated with the [`context.annotate`](https://vitest.dev/guide/test-context#annotate) method. When `annotate` is invoked, Vitest serialises it and sends the same attachment to the main thread where reporter can interact with it.

If the path is specified, Vitest stores it in a separate directory (configured by [`attachmentsDir`](https://vitest.dev/config/#attachmentsdir)) and modifies the `path` property to reference it.
...
The `onTestCaseArtifactRecord` hook is associated with the [`recordArtifact`](https://vitest.dev/api/advanced/artifacts#recordartifact) utility. When `recordArtifact` is invoked, Vitest serialises it and sends the same attachment to the main thread where reporter can interact with it.

If the path is specified, Vitest stores it in a separate directory (configured by [`attachmentsDir`](https://vitest.dev/config/#attachmentsdir)) and modifies the `path` property to reference it.

Note: annotations, [even though they're built on top of this feature](https://vitest.dev/api/advanced/artifacts#relationship-with-annotations), won't hit this hook and won't appear in the `task.artifacts` array for backwards compatibility reasons until the next major version.
