...
description: 'This guide describes the advanced Node.js API. If you just want to define projects, follow the "Test Projects" guide.'
---

Source URL: https://vitest.dev/api/advanced/test-project

# TestProject 3.0.0 {#testproject}

::: warning
This guide describes the advanced Node.js API. If you just want to define projects, follow the ["Test Projects"](https://vitest.dev/guide/projects) guide.
:::
...
::: info
If the [root project](https://vitest.dev/api/advanced/vitest#getroottestproject) is not part of user projects, its `name` will not be resolved.
:::

## vitest

`vitest` references the global [`Vitest`](https://vitest.dev/api/advanced/vitest) process.
...
The test config that [`Vitest`](https://vitest.dev/api/advanced/vitest) was initialized with. If this is the [root project](https://vitest.dev/api/advanced/vitest#getroottestproject), `globalConfig` and `config` will reference the same object. This config is useful for values that cannot be set on the project level, like `coverage` or `reporters`.
...
::: tip
This method is also available to [global setup files](https://vitest.dev/config/#globalsetup) for cases where you cannot use the public API:
...
Create a [test specification](https://vitest.dev/api/advanced/test-specification) that can be used in [`vitest.runTestSpecifications`](https://vitest.dev/api/advanced/vitest#runtestspecifications). Specification scopes the test file to a specific `project` and test `locations` (optional). Test [locations](https://vitest.dev/api/advanced/test-case#location) are code lines where the test is defined in the source code. If locations are provided, Vitest will only run tests defined on those lines. Note that if [`testNamePattern`](https://vitest.dev/config/#testnamepattern) is defined, then it will also be applied.
...
::: warning
`createSpecification` expects resolved [module ID](https://vitest.dev/api/advanced/test-specification#moduleid). It doesn't auto-resolve the file or check that it exists on the file system.
...
This method accepts `filters`. Filters can only a part of the file path, unlike in other methods on the [`Vitest`](https://vitest.dev/api/advanced/vitest) instance:
...
This is a shorthand for [`project.vitest.onTestsRerun`](https://vitest.dev/api/advanced/vitest#ontestsrerun). It accepts a callback that will be awaited when the tests have been scheduled to rerun (usually, due to a file change).
