...
description: "This is an advanced API. As a user, you most likely want to use test annotations to add notes or context to your tests instead. This is primarily used..."
---

Source URL: https://vitest.dev/api/advanced/artifacts

# Test Artifacts 4.0.11

::: warning
This is an advanced API. As a user, you most likely want to use [test annotations](https://vitest.dev/guide/test-annotations) to add notes or context to your tests instead. This is primarily used internally and by library authors.
:::
...
- Internal use ([`annotate`](https://vitest.dev/guide/test-annotations) is built on top of the artifact system)
...
The `recordArtifact` function records an artifact during test execution and returns it. It expects a [task](https://vitest.dev/api/advanced/runner#tasks) as the first parameter and an object assignable to [`TestArtifact`](#testartifact) as the second.
...
When an artifact is recorded on a test, it emits an `onTestArtifactRecord` runner event and a [`onTestCaseArtifactRecord` reporter event](https://vitest.dev/api/advanced/reporters#ontestcaseartifactrecord). To retrieve recorded artifacts from a test case, use the [`artifacts()`](https://vitest.dev/api/advanced/test-case#artifacts) method.
