---
title: "Vitest"
description: "Vitest instance requires the current test mode. It can be either:"
---

Source URL: https://vitest.dev/api/advanced/vitest
...
Global state stores information about the current tests. It uses the same API from `@vitest/runner` by default, but we recommend using the [Reported Tasks API](https://vitest.dev/api/advanced/reporters#reported-tasks) instead by calling `state.getReportedEntity()` on the `@vitest/runner` API:
...
An array of [test projects](https://vitest.dev/api/advanced/test-project) that belong to user's projects. If the user did not specify a them, this array will only contain a [root project](#getrootproject).
...
::: warning
Technically, `provide` is a method of [`TestProject`](https://vitest.dev/api/advanced/test-project), so it is limited to the specific project. However, all projects inherit the values from the root project which makes `vitest.provide` universal way of passing down values to tests.
:::
...
This method constructs new [test specifications](https://vitest.dev/api/advanced/test-specification) by collecting every test in all projects with [`project.globTestFiles`](https://vitest.dev/api/advanced/test-project#globtestfiles). It accepts string filters to match the test files - these are the same filters that [CLI supports](https://vitest.dev/guide/filtering#cli).
...
This method resolves every test specification by calling [`project.globTestFiles`](https://vitest.dev/api/advanced/test-project#globtestfiles). It accepts string filters to match the test files - these are the same filters that [CLI supports](https://vitest.dev/guide/filtering#cli). If `--changed` flag was specified, the list will be filtered to include only files that changed. `getRelevantTestSpecifications` doesn't run any test files.
...
This method is called automatically by [`startVitest`](https://vitest.dev/guide/advanced/tests) if `config.mergeReports` is set.
...
Execute test files without running test callbacks. `collect` returns unhandled errors and an array of [test modules](https://vitest.dev/api/advanced/test-module). It accepts string filters to match the test files - these are the same filters that [CLI supports](https://vitest.dev/guide/filtering#cli).

This method resolves tests specifications based on the config `include`, `exclude`, and `includeSource` values. Read more at [`project.globTestFiles`](https://vitest.dev/api/advanced/test-project#globtestfiles). If `--changed` flag was specified, the list will be filtered to include only files that changed.
...
Initialize reporters, the coverage provider, and run tests. This method accepts string filters to match the test files - these are the same filters that [CLI supports](https://vitest.dev/guide/filtering#cli).
...
This method is called automatically by [`startVitest`](https://vitest.dev/guide/advanced/tests) if `config.mergeReports` and `config.standalone` are not set.
...
Internally, this method is called only if [`--standalone`](https://vitest.dev/guide/cli#standalone) flag is enabled.

::: warning
This method should not be called if [`vitest.start()`](#start) is also invoked.
:::

This method is called automatically by [`startVitest`](https://vitest.dev/guide/advanced/tests) if `config.standalone` is set.
...
This method can return already cached specifications based on the `moduleId` and `pool`. But note that [`project.createSpecification`](https://vitest.dev/api/advanced/test-project#createspecification) always returns a new instance and it's not cached automatically. However, specifications are automatically cached when [`runTestSpecifications`](#runtestspecifications) is called.
...
This method runs every test based on the received [specifications](https://vitest.dev/api/advanced/test-specification). The second argument, `allTestsRun`, is used by the coverage provider to determine if it needs to include uncovered files in report.
...
Execute test files without running test callbacks. `collectTests` returns unhandled errors and an array of [test modules](https://vitest.dev/api/advanced/test-module).
...
This methods overrides the global [test name pattern](https://vitest.dev/config/#testnamepattern).
...
This methods resets the [test name pattern](https://vitest.dev/config/#testnamepattern). It means Vitest won't skip any tests now.
...
This method will also forcefully call `process.exit()` if the process is still active after [`config.teardownTimeout`](https://vitest.dev/config/#teardowntimeout) milliseconds.
...
Check if the name matches the current [project filter](https://vitest.dev/guide/cli#project). If there is no project filter, this will always return `true`.
...
::: warning
This method will also clean all previous reports if [`coverage.clean`](https://vitest.dev/config/#coverage-clean) is not set to `false`.
:::
...
Deletes all Vitest caches, including [`experimental.fsModuleCache`](https://vitest.dev/config/experimental#experimental-fsmodulecache).
...
Returns module's diagnostic. If [`testModule`](https://vitest.dev/api/advanced/test-module) is not provided, `selfTime` and `totalTime` will be aggregated across all tests that were running the last time. If the module was not transformed or executed, the diagnostic will be empty.

::: warning
At the moment, the [browser](https://vitest.dev/guide/browser/) modules are not supported.
:::
