...
Source URL: https://vitest.dev/api/expect
...
::: warning
`expect` has no effect on testing types, if the expression doesn't have a type error. If you want to use Vitest as [type checker](https://vitest.dev/guide/testing-types), use [`expectTypeOf`](https://vitest.dev/api/expect-typeof) or [`assertType`](https://vitest.dev/api/assert-type).
:::
...
Vitest reexports chai's [`assert` API](https://www.chaijs.com/api/assert/) as `expect.assert` for convenience. You can see the supported methods on the [Assert API page](https://vitest.dev/api/assert).
...
:::tip
If you want to know more, checkout [guide on extending matchers](https://vitest.dev/guide/extending-matchers).
:::
