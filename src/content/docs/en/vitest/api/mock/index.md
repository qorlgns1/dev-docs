---
title: "Mocks"
description: "You can create a mock function or a class to track its execution with the  method. If you want to track a property on an already created object, you c..."
---

Source URL: https://vitest.dev/api/mock
...
You should use mock assertions (e.g., [`toHaveBeenCalled`](https://vitest.dev/api/expect#tohavebeencalled)) on [`expect`](https://vitest.dev/api/expect) to assert mock results. This API reference describes available properties and methods to manipulate mock behavior.
...
Does what [`mockReset`](#mockreset) does and restores the original descriptors of spied-on objects, if the mock was created with [`vi.spyOn`](https://vitest.dev/api/vi#vi-spyon).
...
Note that Vitest always stores objects by reference in all properies of the `mock` state. This means that if the properties were changed by your code, then some assertions like [`.toHaveBeenCalledWith`](https://vitest.dev/api/expect#tohavebeencalledwith) will not pass:
