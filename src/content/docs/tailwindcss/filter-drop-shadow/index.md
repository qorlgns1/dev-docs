---
title: "filter: drop-shadow() - 필터 - Tailwind CSS"
description: "요소에 드롭 섀도를 추가하려면 drop-shadow-sm 및 drop-shadow-xl 같은 유틸리티를 사용하세요:"
---

출처 URL: https://tailwindcss.com/docs/filter-drop-shadow

# filter: drop-shadow() - 필터 - Tailwind CSS
...
요소에 드롭 섀도를 추가하려면 `drop-shadow-sm` 및 `drop-shadow-xl` 같은 유틸리티를 사용하세요:
...
이는 텍스트나 SVG 요소처럼 불규칙한 형태에 그림자를 적용할 때 유용합니다. 일반적인 요소에 그림자를 적용하려면 대신 [box shadow](https://tailwindcss.com/docs/box-shadow)를 사용하는 것이 좋습니다.

- 불투명도 변경

드롭 섀도의 불투명도를 조정하려면 opacity modifier를 사용하세요:
...
기본 드롭 섀도 불투명도는 꽤 낮고(15% 이하), 불투명도를(예: 50%) 높이면 드롭 섀도가 더 뚜렷해집니다.

- 그림자 색상 설정

드롭 섀도의 색상을 변경하려면 `drop-shadow-indigo-500` 및 `drop-shadow-cyan-500/50` 같은 유틸리티를 사용하세요:
...
기본적으로 색상이 있는 그림자의 불투명도는 100%이지만, opacity modifier를 사용해 조정할 수 있습니다.

- 드롭 섀도 제거

요소에 적용된 기존 드롭 섀도를 제거하려면 `drop-shadow-none` 유틸리티를 사용하세요:
...
완전히 사용자 지정한 값으로 드롭 섀도를 설정하려면 `drop-shadow-[<value>]` 문법을 사용하세요:

```
    <svg class="drop-shadow-[0_35px_35px_rgba(0,0,0,0.25)] ...">  <!-- ... --></svg>
```

CSS 변수의 경우 `drop-shadow-(<custom-property>)` 문법도 사용할 수 있습니다:

```
    <svg class="drop-shadow-(--my-drop-shadow) ...">  <!-- ... --></svg>
```

이는 `var()` 함수를 자동으로 추가해 주는 `drop-shadow-[var(<custom-property>)]`의 축약형입니다.

- 반응형 디자인

`filter: drop-shadow()` 유틸리티 앞에 `md:` 같은 breakpoint variant를 붙이면 중간 이상 화면 크기에서만 해당 유틸리티가 적용됩니다:

```
    <svg class="drop-shadow-md md:drop-shadow-xl ...">  <!-- ... --></svg>
```

variant 사용에 대한 자세한 내용은 [variants documentation](https://tailwindcss.com/docs/hover-focus-and-other-states)에서 확인하세요.

## 테마 사용자 지정

- 드롭 섀도 사용자 지정

프로젝트에서 드롭 섀도 유틸리티를 사용자 지정하려면 `--drop-shadow-*` theme variables를 사용하세요:

```
    @theme {  --drop-shadow-3xl: 0 35px 35px rgba(0, 0, 0, 0.25); }
```

이제 마크업에서 `drop-shadow-3xl` 유틸리티를 사용할 수 있습니다:
...
프로젝트에서 색상 유틸리티를 사용자 지정하려면 `--color-*` theme variables를 사용하세요:

```
    @theme {  --color-regal-blue: #243c5a; }
```

이제 마크업에서 `drop-shadow-regal-blue` 유틸리티를 사용할 수 있습니다:
