---
title: "text-decoration-color - 타이포그래피 - Tailwind CSS"
description: "decoration-sky-500 및 decoration-pink-500 같은 유틸리티를 사용해 요소의 text decoration 색상을 변경하세요:"
---

출처 URL: https://tailwindcss.com/docs/text-decoration-color

# text-decoration-color - 타이포그래피 - Tailwind CSS
...
`decoration-sky-500` 및 `decoration-pink-500` 같은 유틸리티를 사용해 요소의 [text decoration](https://tailwindcss.com/docs/text-decoration-line) 색상을 변경하세요:

저는 Tattooine에 거주하는 우주공학 엔지니어 Derek입니다. [My Company, Inc](https://tailwindcss.com/docs/text-decoration-color)에서 X-Wing을 만드는 일을 좋아합니다. 일할 때 외에는 [pod-racing 관람](https://tailwindcss.com/docs/text-decoration-color)을 즐기고 [light-saber](https://tailwindcss.com/docs/text-decoration-color) 결투를 합니다.
...
색상 불투명도 modifier를 사용해 요소의 텍스트 장식 색상 불투명도를 제어하세요:
...
`decoration-[<value>]` 문법을 사용해 완전히 사용자 정의한 값으로 텍스트 장식 색상을 설정하세요:

```
    <p class="decoration-[#50d71e] ...">  Lorem ipsum dolor sit amet...</p>
```

CSS 변수의 경우 `decoration-(<custom-property>)` 문법도 사용할 수 있습니다:

```
    <p class="decoration-(--my-color) ...">  Lorem ipsum dolor sit amet...</p>
```

이는 `var()` 함수를 자동으로 추가해 주는 `decoration-[var(<custom-property>)]`의 축약형입니다.

- hover에 적용하기

`text-decoration-color` 유틸리티 앞에 `hover:*` 같은 variant를 붙여 해당 상태에서만 유틸리티가 적용되도록 하세요:

예상 동작을 보려면 텍스트 위에 마우스를 올려보세요
...
`text-decoration-color` 유틸리티 앞에 `md:` 같은 breakpoint variant를 붙여 중간 크기 화면 이상에서만 유틸리티가 적용되도록 하세요:
...
이제 마크업에서 `decoration-regal-blue` 유틸리티를 사용할 수 있습니다:
