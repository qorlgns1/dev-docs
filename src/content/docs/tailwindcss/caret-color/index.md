---
title: "caret-color - 인터랙티비티 - Tailwind CSS"
description: "caret-rose-500 및 caret-lime-600 같은 유틸리티를 사용해 텍스트 입력 커서 색상을 변경하세요:"
---

# caret-color - 인터랙티비티 - Tailwind CSS
...
`caret-rose-500` 및 `caret-lime-600` 같은 유틸리티를 사용해 텍스트 입력 커서 색상을 변경하세요:

새 caret 색상을 보려면 textarea에 포커스를 맞추세요
...
완전히 사용자 지정한 값을 기반으로 caret 색상을 설정하려면 `caret-[<value>]` 구문을 사용하세요:

```
    <textarea class="caret-[#50d71e] ..."></textarea>
```

CSS 변수의 경우 `caret-(<custom-property>)` 구문도 사용할 수 있습니다:

```
    <textarea class="caret-(--my-caret-color) ..."></textarea>
```

이는 `var()` 함수를 자동으로 추가해 주는 `caret-[var(<custom-property>)]`의 축약형입니다.

- 반응형 디자인

`md:` 같은 브레이크포인트 variant를 `caret-color` 유틸리티 앞에 붙이면 중간 화면 크기 이상에서만 해당 유틸리티가 적용됩니다:
...
이제 마크업에서 `caret-regal-blue` 유틸리티를 사용할 수 있습니다:

```
    <textarea class="caret-regal-blue"></textarea>
```

테마를 사용자 지정하는 방법은 [테마 문서](https://tailwindcss.com/docs/theme#customizing-your-theme)에서 자세히 알아보세요.
