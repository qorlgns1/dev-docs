...
description: "Tailwind CSS를 Vite 플러그인으로 설치하는 것은 Laravel, SvelteKit, React Router, Nuxt, SolidJS 같은 프레임워크와 통합하는 가장 매끄러운 방법입니다."
---
...
Tailwind CSS를 Vite 플러그인으로 설치하는 것은 Laravel, SvelteKit, React Router, Nuxt, SolidJS 같은 프레임워크와 통합하는 가장 매끄러운 방법입니다.
...
`npm run dev` 또는 `package.json` 파일에 구성된 명령어로 빌드 프로세스를 실행하세요.
...
컴파일된 CSS가 `<head>`에 포함되어 있는지 확인한 다음 _(프레임워크가 이 작업을 대신 처리할 수도 있습니다)_ Tailwind의 유틸리티 클래스를 사용해 콘텐츠 스타일링을 시작하세요.
...
**막히셨나요?** Tailwind를 Vite와 함께 설정하는 방법은 빌드 도구마다 조금씩 다를 수 있습니다. 현재 구성에 더 구체적인 안내가 있는지 프레임워크 가이드를 확인해 보세요.
