---
title: "box-shadow - 효과 - Tailwind CSS"
description: "shadow-sm 및 shadow-lg 같은 유틸리티를 사용해 요소에 크기가 다른 외부 박스 섀도를 적용할 수 있습니다:"
---

# box-shadow - 효과 - Tailwind CSS
...
`shadow-sm` 및 `shadow-lg` 같은 유틸리티를 사용해 요소에 크기가 다른 외부 박스 섀도를 적용할 수 있습니다:
...
불투명도 modifier를 사용해 박스 섀도의 불투명도를 조정할 수 있습니다:
...
기본 박스 섀도 불투명도는 꽤 낮게 설정되어 있으므로(25% 이하), 불투명도를(예: 50%) 높이면 박스 섀도가 더 뚜렷해집니다.

- 섀도 색상 설정

`shadow-indigo-500` 및 `shadow-cyan-500/50` 같은 유틸리티를 사용해 박스 섀도의 색상을 변경할 수 있습니다:
...
기본적으로 색상이 적용된 섀도의 불투명도는 100%이지만, 불투명도 modifier를 사용해 조정할 수 있습니다.

- 내부 섀도 추가

`inset-shadow-xs` 및 `inset-shadow-sm` 같은 유틸리티를 사용해 요소에 내부 박스 섀도를 적용할 수 있습니다:
...
`inset-shadow-sm/50`처럼 불투명도 modifier를 사용해 내부 섀도의 불투명도를 조정할 수 있습니다. 기본 내부 섀도 불투명도는 꽤 낮게(5%) 설정되어 있으므로, 불투명도를(예: 50%) 높이면 내부 섀도가 더 뚜렷해집니다.

- 내부 섀도 색상 설정

`inset-shadow-indigo-500` 및 `inset-shadow-cyan-500/50` 같은 유틸리티를 사용해 내부 박스 섀도의 색상을 변경할 수 있습니다:
...
기본적으로 colored shadows의 불투명도는 100%이지만, opacity modifier를 사용해 조정할 수 있습니다.

- 링 추가하기

`ring` 또는 `ring-2`, `ring-4` 같은 `ring-<number>` 유틸리티를 사용해 요소에 단색 box-shadow를 적용할 수 있습니다:
...
기본적으로 링은 적용된 요소의 `currentColor`와 동일한 색상을 사용합니다.

- 링 색상 설정하기

`ring-indigo-500`, `ring-cyan-500/50` 같은 유틸리티를 사용해 링 색상을 변경할 수 있습니다:
...
기본적으로 링의 불투명도는 100%이지만, opacity modifier를 사용해 조정할 수 있습니다.

- 인셋 링 추가하기

`inset-ring` 또는 `inset-ring-2`, `inset-ring-4` 같은 `inset-ring-<number>` 유틸리티를 사용해 요소에 단색 inset box-shadow를 적용할 수 있습니다:
...
기본적으로 인셋 링은 적용된 요소의 `currentColor`와 동일한 색상을 사용합니다.

- 인셋 링 색상 설정하기

`inset-ring-indigo-500`, `inset-ring-cyan-500/50` 같은 유틸리티를 사용해 인셋 링의 색상을 변경할 수 있습니다:
...
기본적으로 인셋 링의 불투명도는 100%이지만, opacity modifier를 사용해 조정할 수 있습니다.

- 박스 섀도 제거하기

`shadow-none`, `inset-shadow-none`,`ring-0`, `inset-ring-0` 유틸리티를 사용해 요소의 기존 box shadow를 제거할 수 있습니다:
...
`shadow-[<value>]`,`inset-shadow-[<value>]`,`ring-[<value>]`, `inset-ring-[<value>]` 같은 유틸리티를 사용해 완전히 사용자 지정한 값으로 box shadow를 설정할 수 있습니다:

```
    <div class="shadow-[0_35px_35px_rgba(0,0,0,0.25)] ...">  <!-- ... --></div>
```

CSS 변수의 경우 `shadow-(<custom-property>)` 문법도 사용할 수 있습니다:

```
    <div class="shadow-(--my-shadow) ...">  <!-- ... --></div>
```

이는 `var()` 함수를 자동으로 추가해 주는 `shadow-[var(<custom-property>)]`의 축약 문법입니다.

- 반응형 디자인

`box-shadow` 유틸리티 앞에 `md:` 같은 breakpoint variant를 붙이면 medium 화면 크기 이상에서만 해당 유틸리티가 적용됩니다:
...
프로젝트에서 box shadow 유틸리티를 사용자 지정하려면 `--shadow-*` 테마 변수를 사용하세요:

```
    @theme {  --shadow-3xl: 0 35px 35px rgba(0, 0, 0, 0.25); }
```

이제 마크업에서 `shadow-3xl` 유틸리티를 사용할 수 있습니다:

```
    <div class="shadow-3xl">  <!-- ... --></div>
```

테마 사용자 지정에 대한 자세한 내용은 [theme documentation](https://tailwindcss.com/docs/theme#customizing-your-theme)을 참고하세요.

- 인셋 섀도 사용자 지정하기

프로젝트에서 inset box shadow 유틸리티를 사용자 지정하려면 `--inset-shadow-*` 테마 변수를 사용하세요:

```
    @theme {  --inset-shadow-md: inset 0 2px 3px rgba(0, 0, 0, 0.25); }
```

이제 마크업에서 `inset-shadow-md` 유틸리티를 사용할 수 있습니다:
...
프로젝트에서 color 유틸리티를 사용자 지정하려면 `--color-*` 테마 변수를 사용하세요:

```
    @theme {  --color-regal-blue: #243c5a; }
```

이제 마크업에서 `shadow-regal-blue`,`inset-shadow-regal-blue`,`ring-regal-blue`, `inset-ring-regal-blue` 같은 유틸리티를 사용할 수 있습니다:
