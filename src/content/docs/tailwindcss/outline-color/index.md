---
title: "outline-color - 테두리 - Tailwind CSS"
description: "outline-inherit| outline-color: inherit;"
---

소스 URL: https://tailwindcss.com/docs/outline-color

# outline-color - 테두리 - Tailwind CSS
...
요소의 outline 색상을 제어하려면 `outline-rose-500` 및 `outline-lime-100` 같은 유틸리티를 사용하세요:
...
요소의 outline 색상 불투명도를 제어하려면 color opacity modifier를 사용하세요:
...
완전히 사용자 정의한 값을 기반으로 outline 색상을 설정하려면 `outline-[<value>]` 구문을 사용하세요:

```
    <div class="outline-[#243c5a] ...">  <!-- ... --></div>
```

CSS 변수의 경우 `outline-(<custom-property>)` 구문도 사용할 수 있습니다:

```
    <div class="outline-(--my-color) ...">  <!-- ... --></div>
```

이는 `outline-[var(<custom-property>)]`의 단축 표기이며, `var()` 함수를 자동으로 추가해 줍니다.

- 반응형 디자인

`outline-color` 유틸리티 앞에 `md:` 같은 breakpoint variant를 접두사로 붙여, 중간 화면 크기 이상에서만 유틸리티가 적용되도록 할 수 있습니다:

```
    <div class="outline md:outline-blue-400 ...">  <!-- ... --></div>
```

variants 사용에 대한 자세한 내용은 [variants documentation](https://tailwindcss.com/docs/hover-focus-and-other-states)를 참고하세요.
...
이제 마크업에서 `outline-regal-blue` 유틸리티를 사용할 수 있습니다:

```
    <div class="outline-regal-blue">  <!-- ... --></div>
```

테마 사용자 정의에 대한 자세한 내용은 [theme documentation](https://tailwindcss.com/docs/theme#customizing-your-theme)를 참고하세요.
