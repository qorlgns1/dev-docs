---
title: "accent-color - 상호작용 - Tailwind CSS"
description: "accent-rose-500, accent-lime-600 같은 유틸리티를 사용해 요소의 강조 색상을 변경하세요:"
---

# accent-color - 상호작용 - Tailwind CSS
...
`accent-rose-500`, `accent-lime-600` 같은 유틸리티를 사용해 요소의 강조 색상을 변경하세요:
...
브라우저의 기본 색상을 재정의해 체크박스나 라디오 그룹 같은 요소를 스타일링할 때 유용합니다.

- 불투명도 변경

색상 불투명도 modifier를 사용해 요소 강조 색상의 불투명도를 제어하세요:
...
강조 색상 불투명도 설정은 브라우저 지원이 제한적이며 현재는 Firefox에서만 동작합니다.

- 사용자 지정 값 사용

`accent-[<value>]` 구문을 사용해 완전히 사용자 지정한 값으로 강조 색상을 설정하세요:

```
    <input class="accent-[#50d71e] ..." type="checkbox" />
```

CSS 변수의 경우 `accent-(<custom-property>)` 구문도 사용할 수 있습니다:

```
    <input class="accent-(--my-accent-color) ..." type="checkbox" />
```

이는 `var()` 함수를 자동으로 추가해 주는 `accent-[var(<custom-property>)]`의 축약형입니다.

- hover에서 적용

`accent-color` 유틸리티 앞에 `hover:*` 같은 variant를 붙여 해당 상태에서만 유틸리티를 적용하세요:
...
`accent-color` 유틸리티 앞에 `md:` 같은 브레이크포인트 variant를 붙여 중간 화면 크기 이상에서만 유틸리티를 적용하세요:
...
`--color-*` 테마 변수를 사용해 프로젝트의 색상 유틸리티를 사용자 지정하세요:

```
    @theme {  --color-regal-blue: #243c5a; }
```

이제 마크업에서 `accent-regal-blue` 유틸리티를 사용할 수 있습니다:

```
    <input class="accent-regal-blue" type="checkbox" />
```

테마 사용자 지정은 [theme 문서](https://tailwindcss.com/docs/theme#customizing-your-theme)에서 더 알아보세요.
