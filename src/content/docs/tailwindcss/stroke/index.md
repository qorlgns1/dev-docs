...
description: "stroke-indigo-500, stroke-lime-600 같은 유틸리티를 사용해 SVG의 스트로크 색상을 변경할 수 있습니다:"
---
...
`stroke-indigo-500`, `stroke-lime-600` 같은 유틸리티를 사용해 SVG의 스트로크 색상을 변경할 수 있습니다:
...
버튼 위에 마우스를 올려 스트로크 색상이 바뀌는 것을 확인하세요
...
`stroke-[<value>]` 구문을 사용하면 완전히 사용자 정의한 값을 기반으로 스트로크 색상을 설정할 수 있습니다:

```
    <svg class="stroke-[#243c5a] ...">  <!-- ... --></svg>
```

CSS 변수의 경우 `stroke-(<custom-property>)` 구문도 사용할 수 있습니다:

```
    <svg class="stroke-(--my-stroke-color) ...">  <!-- ... --></svg>
```

이는 `var()` 함수를 자동으로 추가해 주는 `stroke-[var(<custom-property>)]`의 축약형입니다.

- 반응형 디자인

`md:` 같은 브레이크포인트 variant를 `stroke` 유틸리티 앞에 붙이면 중간 화면 크기 이상에서만 해당 유틸리티를 적용할 수 있습니다:
...
`--color-*` 테마 변수를 사용해 프로젝트의 색상 유틸리티를 커스터마이징할 수 있습니다:

```
    @theme {  --color-regal-blue: #243c5a; }
```

이제 마크업에서 `stroke-regal-blue` 유틸리티를 사용할 수 있습니다:
