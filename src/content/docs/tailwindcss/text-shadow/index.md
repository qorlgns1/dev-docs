---
title: "text-shadow - 효과 - Tailwind CSS"
description: "텍스트 요소에 서로 다른 크기의 텍스트 그림자를 적용하려면 text-shadow-sm, shadow-lg 같은 유틸리티를 사용하세요:"
---

출처 URL: https://tailwindcss.com/docs/text-shadow

# text-shadow - 효과 - Tailwind CSS
...
텍스트 요소에 서로 다른 크기의 텍스트 그림자를 적용하려면 `text-shadow-sm`, `shadow-lg` 같은 유틸리티를 사용하세요:
...
텍스트 그림자의 불투명도를 조정하려면 opacity modifier를 사용하세요:
...
기본 텍스트 그림자 불투명도는 꽤 낮게(20% 이하) 설정되어 있으므로, 불투명도를 (예: 50%) 높이면 텍스트 그림자가 더 뚜렷해집니다.

- 그림자 색상 설정

텍스트 그림자 색상을 변경하려면 `text-shadow-indigo-500`, `text-shadow-cyan-500/50` 같은 유틸리티를 사용하세요:
...
요소에 이미 적용된 텍스트 그림자를 제거하려면 `text-shadow-none` 유틸리티를 사용하세요:
...
완전히 사용자 정의된 값으로 텍스트 그림자를 설정하려면 `text-shadow-[<value>]` 구문을 사용하세요:

```
    <p class="text-shadow-[0_35px_35px_rgb(0_0_0_/_0.25)] ...">  Lorem ipsum dolor sit amet...</p>
```

CSS 변수의 경우 `text-shadow-(<custom-property>)` 구문도 사용할 수 있습니다:

```
    <p class="text-shadow-(--my-text-shadow) ...">  Lorem ipsum dolor sit amet...</p>
```

이는 `var()` 함수를 자동으로 추가해 주는 `text-shadow-[var(<custom-property>)]`의 단축 표기입니다.

- 반응형 디자인

중간 화면 크기 이상에서만 유틸리티를 적용하려면 `text-shadow` 유틸리티 앞에 `md:` 같은 브레이크포인트 variant를 붙이세요:
...
프로젝트에서 텍스트 그림자 유틸리티를 사용자 정의하려면 `--text-shadow-*` 테마 변수를 사용하세요:

```
    @theme {  --text-shadow-xl: 0 35px 35px rgb(0, 0, 0 / 0.25); }
```

이제 마크업에서 `text-shadow-xl` 유틸리티를 사용할 수 있습니다:
...
프로젝트에서 색상 유틸리티를 사용자 정의하려면 `--color-*` 테마 변수를 사용하세요:

```
    @theme {  --color-regal-blue: #243c5a; }
```

이제 마크업에서 `text-shadow-regal-blue` 유틸리티를 사용할 수 있습니다:
