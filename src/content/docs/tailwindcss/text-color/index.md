---
title: "색상 - 타이포그래피 - Tailwind CSS"
description: "요소의 텍스트 색상을 제어하려면 text-blue-600, text-sky-400 같은 유틸리티를 사용하세요:"
---

# 색상 - 타이포그래피 - Tailwind CSS
...
요소의 텍스트 색상을 제어하려면 `text-blue-600`, `text-sky-400` 같은 유틸리티를 사용하세요:
...
색상 불투명도 modifier를 사용해 요소 텍스트 색상의 불투명도를 제어하세요:
...
완전히 사용자 정의한 값을 기준으로 텍스트 색상을 설정하려면 `text-[<value>]` 구문을 사용하세요:

```
    <p class="text-[#50d71e] ...">  Lorem ipsum dolor sit amet...</p>
```

CSS 변수의 경우 `text-(<custom-property>)` 구문도 사용할 수 있습니다:

```
    <p class="text-(--my-color) ...">  Lorem ipsum dolor sit amet...</p>
```

이는 `var()` 함수를 자동으로 추가해 주는 `text-[var(<custom-property>)]`의 단축 표기입니다.

- hover 시 적용

`color` 유틸리티 앞에 `hover:*` 같은 variant를 붙이면 해당 상태에서만 유틸리티가 적용됩니다:

예상 동작을 확인하려면 텍스트 위에 마우스를 올려보세요

아, 저 [인터넷](https://en.wikipedia.org/wiki/Internet) 좀 해야겠어. 뭐든 다 늦었네!
...
`color` 유틸리티 앞에 `md:` 같은 breakpoint variant를 붙이면 중간 크기 이상의 화면에서만 유틸리티가 적용됩니다:
...
이제 마크업에서 `text-regal-blue` 유틸리티를 사용할 수 있습니다:
