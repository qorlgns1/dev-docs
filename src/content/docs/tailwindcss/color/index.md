---
title: "color - 타이포그래피 - Tailwind CSS"
description: "text-blue-600 및 text-sky-400 같은 유틸리티를 사용해 요소의 텍스트 색상을 제어하세요:"
---

출처 URL: https://tailwindcss.com/docs/color

# color - 타이포그래피 - Tailwind CSS
...
`text-blue-600` 및 `text-sky-400` 같은 유틸리티를 사용해 요소의 텍스트 색상을 제어하세요:
...
색상 불투명도 수정자를 사용해 요소의 텍스트 색상 불투명도를 제어하세요:
...
`text-[<value>]` 구문을 사용해 완전히 사용자 지정한 값으로 텍스트 색상을 설정하세요:
...
`hover:*` 같은 variant를 `color` 유틸리티 앞에 붙이면 해당 상태에서만 유틸리티가 적용됩니다:

예상 동작을 확인하려면 텍스트 위에 마우스를 올려보세요

아, 나도 그 [internet](https://en.wikipedia.org/wiki/Internet) 해야겠어. 모든 게 늦었네!
...
`md:` 같은 브레이크포인트 variant를 `color` 유틸리티 앞에 붙이면 중간 화면 크기 이상에서만 유틸리티가 적용됩니다:
