---
title: "테마 변수 - 핵심 개념 - Tailwind CSS"
description: "Tailwind는 커스텀 디자인을 구축하기 위한 프레임워크이며, 디자인마다 서로 다른 타이포그래피, 색상, 그림자, 브레이크포인트 등이 필요합니다."
---

# 테마 변수 - 핵심 개념 - Tailwind CSS

## 개요

Tailwind는 커스텀 디자인을 구축하기 위한 프레임워크이며, 디자인마다 서로 다른 타이포그래피, 색상, 그림자, 브레이크포인트 등이 필요합니다.

이러한 저수준 디자인 결정은 흔히 _디자인 토큰_ 이라고 하며, Tailwind 프로젝트에서는 그 값을 _테마 변수_ 에 저장합니다.

- 테마 변수란 무엇인가요?

테마 변수는 `@theme` 지시어를 사용해 정의하는 특별한 CSS 변수로, 프로젝트에 어떤 유틸리티 클래스가 존재하는지에 영향을 줍니다.

예를 들어 `--color-mint-500` 같은 테마 변수를 정의해 프로젝트에 새 색상을 추가할 수 있습니다:
...
이제 HTML에서 `bg-mint-500`, `text-mint-500`, `fill-mint-500` 같은 유틸리티 클래스를 사용할 수 있습니다:
...
Tailwind는 테마 변수에 대해 일반 CSS 변수도 생성하므로, 임의 값이나 인라인 스타일에서 디자인 토큰을 참조할 수 있습니다:
...
테마 변수가 다양한 유틸리티 클래스에 어떻게 매핑되는지는 [theme variable namespaces](https://tailwindcss.com/docs/theme#theme-variable-namespaces) 문서를 참고하세요.

#

- 왜 `:root`가 아니라 `@theme`인가요?

테마 변수는 _단순한_ CSS 변수가 아닙니다. HTML에서 사용할 수 있는 새 유틸리티 클래스를 Tailwind가 만들도록 지시하기도 합니다.

일반 CSS 변수보다 더 많은 역할을 하기 때문에, Tailwind는 테마 변수 정의가 항상 명시적이도록 특별한 문법을 사용합니다. 또한 테마 변수는 최상위에서 정의되어야 하며 다른 셀렉터나 미디어 쿼리 아래에 중첩할 수 없는데, 이런 제약도 특별 문법 덕분에 강제할 수 있습니다.

Tailwind 프로젝트에서도 유틸리티 클래스와 연결할 목적이 아닌 변수를 정의할 때는 `:root`로 일반 CSS 변수를 정의하는 방식이 여전히 유용합니다. 디자인 토큰을 유틸리티 클래스에 직접 매핑하려면 `@theme`를 사용하고, 대응 유틸리티 클래스가 필요 없는 일반 CSS 변수는 `:root`를 사용하세요.

- 유틸리티 클래스와의 관계

`flex`, `object-cover` 같은 Tailwind 유틸리티 클래스 일부는 정적이며 프로젝트와 관계없이 항상 동일합니다. 하지만 많은 클래스는 테마 변수에 의해 구동되며, 여러분이 정의한 테마 변수 때문에 존재합니다.

예를 들어 `--font-*` 네임스페이스에서 정의한 테마 변수는 프로젝트에 존재하는 모든 `font-family` 유틸리티를 결정합니다:
...
`font-sans`, `font-serif`, `font-mono` 유틸리티가 기본으로 존재하는 이유는 Tailwind 기본 테마가 `--font-sans`, `--font-serif`, `--font-mono` 테마 변수를 정의하기 때문입니다.

`--font-poppins` 같은 다른 테마 변수를 정의하면, 이에 맞는 `font-poppins` 유틸리티 클래스도 사용할 수 있게 됩니다:
...
이 네임스페이스 안에서는 테마 변수 이름을 원하는 대로 지을 수 있으며, 같은 이름의 대응 유틸리티를 HTML에서 사용할 수 있게 됩니다.

#

- variant와의 관계

일부 테마 변수는 유틸리티가 아니라 variant를 정의하는 데 사용됩니다. 예를 들어 `--breakpoint-*` 네임스페이스의 테마 변수는 프로젝트에서 어떤 반응형 브레이크포인트 variant가 존재하는지 결정합니다:
...
이제 뷰포트가 120rem 이상일 때만 유틸리티가 적용되도록 `3xl:*` variant를 사용할 수 있습니다:
...
테마 변수가 다양한 유틸리티 클래스와 variant에 어떻게 매핑되는지는 [theme variable namespaces](https://tailwindcss.com/docs/theme#theme-variable-namespaces) 문서를 참고하세요.

- 테마 변수 네임스페이스

테마 변수는 _네임스페이스_ 로 정의되며, 각 네임스페이스는 하나 이상의 유틸리티 클래스 API 또는 variant API에 대응됩니다.

이 네임스페이스에 새 테마 변수를 정의하면 프로젝트에서 대응되는 새 유틸리티와 variant를 사용할 수 있게 됩니다:

| Namespace          | 유틸리티 클래스                                                     |
| ------------------ | ------------------------------------------------------------------- |
| `--color-*`        | `bg-red-500`, `text-sky-300` 등 다양한 색상 유틸리티                |
| `--font-*`         | `font-sans` 같은 폰트 패밀리 유틸리티                               |
| `--text-*`         | `text-xl` 같은 폰트 크기 유틸리티                                   |
| `--font-weight-*`  | `font-bold` 같은 폰트 굵기 유틸리티                                 |
| `--tracking-*`     | `tracking-wide` 같은 자간 유틸리티                                  |
| `--leading-*`      | `leading-tight` 같은 줄 높이 유틸리티                               |
| `--breakpoint-*`   | `sm:*` 같은 반응형 브레이크포인트 variant                           |
| `--container-*`    | `@sm:*` 같은 컨테이너 쿼리 variant 및 `max-w-md` 같은 크기 유틸리티 |
| `--spacing-*`      | `px-4`, `max-h-16` 등 다양한 간격/크기 유틸리티                     |
| `--radius-*`       | `rounded-sm` 같은 테두리 반경 유틸리티                              |
| `--shadow-*`       | `shadow-md` 같은 박스 그림자 유틸리티                               |
| `--inset-shadow-*` | `inset-shadow-xs` 같은 안쪽 박스 그림자 유틸리티                    |
| `--drop-shadow-*`  | `drop-shadow-md` 같은 드롭 섀도 필터 유틸리티                       |
| `--blur-*`         | `blur-md` 같은 블러 필터 유틸리티                                   |
| `--perspective-*`  | `perspective-near` 같은 원근 유틸리티                               |
| `--aspect-*`       | `aspect-video` 같은 종횡비 유틸리티                                 |
| `--ease-*`         | `ease-out` 같은 전환 타이밍 함수 유틸리티                           |
| `--animate-*`      | `animate-spin` 같은 애니메이션 유틸리티                             |

기본 테마 변수 전체 목록은 [default theme variable reference](https://tailwindcss.com/docs/theme#default-theme-variable-reference)를 참고하세요.

- 기본 테마 변수

CSS 파일 맨 위에서 `tailwindcss`를 import하면 시작에 필요한 기본 테마 변수 세트가 포함됩니다.

`tailwindcss`를 import할 때 실제로 import되는 내용은 다음과 같습니다:
...
해당 `theme.css` 파일에는 기본 색상 팔레트, 타입 스케일, 그림자, 폰트 등이 포함되어 있습니다:
...
그래서 `bg-red-200`, `font-serif`, `shadow-sm` 같은 유틸리티가 기본 제공됩니다. 이는 `flex-col`, `pointer-events-none`처럼 프레임워크에 하드코딩된 것이 아니라 기본 테마에 의해 구동되기 때문입니다.

기본 테마 변수 전체 목록은 [default theme variable reference](https://tailwindcss.com/docs/theme#default-theme-variable-reference)를 참고하세요.

## 테마 커스터마이징

기본 테마 변수는 매우 범용적이어서 크게 다른 디자인도 만들 수 있지만, 어디까지나 출발점입니다. 색상 팔레트, 폰트, 그림자 등을 커스터마이징해 의도한 디자인을 정확히 구현하는 경우가 매우 흔합니다.

- 기본 테마 확장하기

`@theme`를 사용해 새 테마 변수를 정의하고 기본 테마를 확장하세요:
...
이렇게 하면 기본 `font-sans`나 `font-mono`처럼 HTML에서 사용할 수 있는 새로운 `font-script` 유틸리티 클래스가 생깁니다:
...
기본 테마 변수 값을 덮어쓰려면 `@theme` 안에서 해당 값을 다시 정의하세요:
...
이제 `sm:*` variant는 기본 40rem 뷰포트 크기 대신 30rem에서 트리거됩니다:
...
기본 테마의 전체 네임스페이스를 완전히 덮어쓰려면, 특수한 별표 문법을 사용해 전체 네임스페이스를 `initial`로 설정하세요:
...
이렇게 하면 해당 네임스페이스를 사용하는 기본 유틸리티 전체 _(예: `bg-red-500`)_ 가 제거되고, 커스텀 값 _(예: `bg-midnight`)_ 만 사용할 수 있게 됩니다.

테마 변수가 다양한 유틸리티 클래스와 variant에 어떻게 매핑되는지는 [theme variable namespaces](https://tailwindcss.com/docs/theme#theme-variable-namespaces) 문서를 참고하세요.

- 커스텀 테마 사용하기

기본 테마를 완전히 비활성화하고 커스텀 값만 사용하려면 전역 테마 변수 네임스페이스를 `initial`로 설정하세요:
...
이제 테마 변수로 구동되는 기본 유틸리티 클래스는 사용할 수 없고, `font-body`, `text-dusk`처럼 커스텀 테마 변수에 맞는 유틸리티 클래스만 사용할 수 있습니다.

- 애니메이션 keyframes 정의하기

생성된 CSS에 포함하려면 `--animate-*` 테마 변수용 `@keyframes` 규칙을 `@theme` 내부에 정의하세요:
...
`--animate-*` 테마 변수를 추가하지 않아도 커스텀 `@keyframes` 규칙을 항상 포함하고 싶다면, 대신 `@theme` 밖에서 정의하세요.

- 다른 변수 참조하기

다른 변수를 참조하는 테마 변수를 정의할 때는 `inline` 옵션을 사용하세요:
...
`inline` 옵션을 사용하면 유틸리티 클래스는 실제 테마 변수를 참조하는 대신 테마 변수의 _값_ 을 사용합니다:
...
`inline`을 사용하지 않으면 CSS에서 변수가 해석되는 방식 때문에 유틸리티 클래스가 예상과 다른 값으로 해석될 수 있습니다.

예를 들어 다음 텍스트는 예상과 달리 `Inter`를 사용하지 않고 `sans-serif`로 폴백됩니다:
...
이는 `var(--font-sans)`가 `--font-sans`가 정의된 위치 _( `#parent` )_ 에서 해석되기 때문이며, `--font-inter`는 트리의 더 깊은 위치 _( `#child` )_ 에서야 정의되므로 그 위치에는 값이 없습니다.

- 모든 CSS 변수 생성하기

기본적으로는 사용된 CSS 변수만 최종 CSS 출력에 생성됩니다. 모든 CSS 변수를 항상 생성하려면 `static` 테마 옵션을 사용할 수 있습니다:
...
테마 변수는 CSS로 정의되므로 프로젝트 간 공유도 간단합니다. 각 프로젝트에서 import할 수 있도록 별도 CSS 파일에 모아두면 됩니다:
...
그다음 `@import`를 사용해 다른 프로젝트에 테마 변수를 포함할 수 있습니다:
...
이처럼 공유 테마 변수는 모노레포에서 별도 패키지로 둘 수 있고, NPM에 배포한 뒤 다른 서드파티 CSS 파일처럼 import할 수도 있습니다.

## 테마 변수 사용하기

CSS를 컴파일하면 모든 테마 변수가 일반 CSS 변수로 변환됩니다:
...
덕분에 커스텀 CSS나 인라인 스타일 어디에서든 디자인 토큰을 쉽게 참조할 수 있습니다.

- 커스텀 CSS에서

같은 값을 사용해야 하는 커스텀 CSS를 작성할 때, 테마 변수를 사용해 디자인 토큰에 접근하세요:
...
이 방식은 데이터베이스나 API에서 온 Markdown 콘텐츠처럼, 내가 제어하지 않는 HTML을 렌더링해 스타일링할 때 특히 유용합니다.

- 임의 값에서

테마 변수를 임의 값에 사용하는 것은 유용하며, 특히 `calc()` 함수와 함께 쓸 때 효과적입니다.
...
위 예시에서는 중첩된 inset 요소가 동심원 형태의 border radius를 갖도록 `--radius-xl` 값에서 1px를 빼고 있습니다.

- JavaScript에서 참조하기

대부분의 경우 JS에서 테마 변수를 참조해야 할 때는 다른 CSS 값처럼 CSS 변수를 직접 사용하면 됩니다.

예를 들어 React에서 널리 쓰이는 [Motion](https://motion.dev/docs/react-quick-start) 라이브러리는 CSS 변수 값으로 애니메이션 시작/종료를 지정할 수 있습니다:
...
JS에서 해석된 CSS 변수 값이 필요하면 `getComputedStyle`을 사용해 문서 루트의 테마 변수 값을 가져올 수 있습니다:
...
참고용으로, 프로젝트에 Tailwind CSS를 import할 때 기본 포함되는 테마 변수 전체 목록은 다음과 같습니다:
