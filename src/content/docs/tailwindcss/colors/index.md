---
title: "색상 - 핵심 개념 - Tailwind CSS"
description: "Tailwind CSS는 전문가 디자이너가 세심하게 설계한 방대하고 아름다운 색상 팔레트를 기본으로 제공하며, 다양한 디자인 스타일에 폭넓게 활용할 수 있습니다."
---

출처 URL: https://tailwindcss.com/docs/colors

# 색상 - 핵심 개념 - Tailwind CSS

Tailwind CSS는 전문가 디자이너가 세심하게 설계한 방대하고 아름다운 색상 팔레트를 기본으로 제공하며, 다양한 디자인 스타일에 폭넓게 활용할 수 있습니다.
...
OKLCH 값을 복사하려면 클릭하고, 가장 가까운 hex 값을 복사하려면 shift+click 하세요.

기본 팔레트의 모든 색상은 11단계로 구성되며, 50이 가장 밝고 950이 가장 어둡습니다:
...
전체 색상 팔레트는 [background color](https://tailwindcss.com/docs/background-color), [border color](https://tailwindcss.com/docs/border-color), [fill](https://tailwindcss.com/docs/fill), [caret color](https://tailwindcss.com/docs/caret-color) 등 모든 색상 관련 유틸리티에서 사용할 수 있습니다.

## 색상 다루기

- 색상 유틸리티 사용하기

`bg-white`, `border-pink-300`, `text-gray-950` 같은 색상 유틸리티를 사용해 디자인 요소의 다양한 색상 속성을 설정할 수 있습니다:

Tom Watson이 로고 리디자인에서 회원님을 언급했습니다
...
다음은 색상 팔레트를 사용하는 유틸리티 전체 목록입니다:

| Utility          | Description                                                                                                     |
| ---------------- | --------------------------------------------------------------------------------------------------------------- |
| `bg-*`           | 요소의 [background color](https://tailwindcss.com/docs/background-color)를 설정합니다                           |
| `text-*`         | 요소의 [text color](https://tailwindcss.com/docs/text-color)를 설정합니다                                       |
| `decoration-*`   | 요소의 [text decoration color](https://tailwindcss.com/docs/text-decoration-color)를 설정합니다                 |
| `border-*`       | 요소의 [border color](https://tailwindcss.com/docs/border-color)를 설정합니다                                   |
| `outline-*`      | 요소의 [outline color](https://tailwindcss.com/docs/outline-color)를 설정합니다                                 |
| `shadow-*`       | [box shadows](https://tailwindcss.com/docs/box-shadow#setting-the-shadow-color)의 색상을 설정합니다             |
| `inset-shadow-*` | [inset box shadows](https://tailwindcss.com/docs/box-shadow#setting-the-inset-shadow-color)의 색상을 설정합니다 |
| `ring-*`         | [ring shadows](https://tailwindcss.com/docs/box-shadow#setting-the-ring-color)의 색상을 설정합니다              |
| `inset-ring-*`   | [inset ring shadows](https://tailwindcss.com/docs/box-shadow#setting-the-inset-ring-color)의 색상을 설정합니다  |
| `accent-*`       | 폼 컨트롤의 [accent color](https://tailwindcss.com/docs/accent-color)를 설정합니다                              |
| `caret-*`        | 폼 컨트롤의 [caret color](https://tailwindcss.com/docs/caret-color)를 설정합니다                                |
| `fill-*`         | SVG 요소의 [fill color](https://tailwindcss.com/docs/fill)를 설정합니다                                         |
| `stroke-*`       | SVG 요소의 [stroke color](https://tailwindcss.com/docs/stroke)를 설정합니다                                     |

- 불투명도 조정하기

`bg-black/75` 같은 문법으로 색상의 불투명도를 조정할 수 있으며, 여기서 `75`는 색상의 알파 채널을 75%로 설정합니다:
...
이 문법은 임의 값과 CSS 변수 단축 표기도 지원합니다:
...
`dark` 변형을 사용하면 `dark:bg-gray-800`처럼 다크 모드가 활성화됐을 때만 색상이 적용되는 클래스를 작성할 수 있습니다:

라이트 모드

거꾸로도 쓸 수 있습니다

Zero Gravity Pen은 거꾸로를 포함해 어떤 방향에서도 쓸 수 있습니다. 우주 공간에서도 작동합니다.
...
다크 모드 스타일링에 대한 자세한 내용은 [dark mode documentation](https://tailwindcss.com/docs/dark-mode)을 참고하세요.

- CSS에서 참조하기

색상은 `--color-*` 네임스페이스의 CSS 변수로 노출되므로, `--color-blue-500`, `--color-pink-700` 같은 변수로 CSS에서 참조할 수 있습니다:
...
유틸리티 클래스에서 이를 [arbitrary values](https://tailwindcss.com/docs/colors)로 사용할 수도 있습니다:
...
CSS에서 변수를 참조할 때 색상 불투명도를 빠르게 조정할 수 있도록 Tailwind는 특별한 `--alpha()` 함수를 제공합니다:
...
`@theme`를 사용해 프로젝트의 `--color-*` 테마 네임스페이스 아래에 사용자 지정 색상을 추가하세요:
...
이제 기본 색상에 더해 `bg-midnight`, `text-tahiti`, `fill-bermuda` 같은 유틸리티를 프로젝트에서 사용할 수 있습니다.

테마 변수에 대한 자세한 내용은 [theme variables documentation](https://tailwindcss.com/docs/theme)을 참고하세요.

- 기본 색상 재정의하기

같은 이름의 새 테마 변수를 정의해 기본 색상을 재정의할 수 있습니다:
...
해당 색상의 테마 네임스페이스를 `initial`로 설정해 기본 색상을 비활성화할 수 있습니다:
...
이는 사용하지 않을 색상에 대한 CSS 변수를 출력 결과에서 제거할 때 특히 유용합니다.

- 사용자 지정 팔레트 사용하기

`--color-*: initial`을 사용해 모든 기본 색상을 완전히 비활성화하고, 자신만의 사용자 지정 색상 팔레트를 정의할 수 있습니다:
...
다른 색상을 참조하는 색상을 정의할 때는 `@theme inline`을 사용하세요:
...
자세한 내용은 테마 문서의 [referencing other variables](https://tailwindcss.com/docs/theme#referencing-other-variables)를 참고하세요.

## 기본 색상 팔레트 참고

참고용으로 기본 색상과 해당 값의 전체 목록은 다음과 같습니다:
...
예를 들어 `--color-gray-*`를 `--color-slate-*` 스케일로 재정의하는 것처럼, 같은 스케일을 다른 이름으로 재사용하고 싶을 때 유용합니다.
