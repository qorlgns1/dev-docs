...
description: "SVG의 fill 색상을 변경하려면 fill-indigo-500 및 fill-lime-600 같은 유틸리티를 사용하세요:"
---

출처 URL: https://tailwindcss.com/docs/fill
...
SVG의 fill 색상을 변경하려면 `fill-indigo-500` 및 `fill-lime-600` 같은 유틸리티를 사용하세요:

```
    <svg class="fill-blue-500 ...">  <!-- ... --></svg>
```

이는 [Heroicons](https://heroicons.com)와 같은 아이콘 세트를 스타일링할 때 유용합니다.

- 현재 색상 사용하기

`fill-current` 유틸리티를 사용해 fill 색상을 현재 텍스트 색상으로 설정하세요:

버튼 위에 마우스를 올려 fill 색상이 바뀌는 것을 확인해 보세요.
...
`fill-[<value>]` 구문을 사용해 완전히 사용자 지정한 값으로 fill 색상을 설정하세요:

```
    <svg class="fill-[#243c5a] ...">  <!-- ... --></svg>
```

CSS 변수의 경우 `fill-(<custom-property>)` 구문도 사용할 수 있습니다:

```
    <svg class="fill-(--my-fill-color) ...">  <!-- ... --></svg>
```

이는 `var()` 함수를 자동으로 추가해 주는 `fill-[var(<custom-property>)]`의 단축 표기입니다.

- 반응형 디자인

`fill` 유틸리티 앞에 `md:` 같은 브레이크포인트 변형을 붙이면 중간 크기 이상의 화면에서만 유틸리티가 적용됩니다:
...
이제 마크업에서 `fill-regal-blue` 유틸리티를 사용할 수 있습니다:
