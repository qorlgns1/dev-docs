---
title: "background-color - 배경 - Tailwind CSS"
description: "요소의 배경색을 제어하려면 bg-white, bg-indigo-500, bg-transparent 같은 유틸리티를 사용하세요:"
---

출처 URL: https://tailwindcss.com/docs/background-color

# background-color - 배경 - Tailwind CSS
...
요소의 배경색을 제어하려면 `bg-white`, `bg-indigo-500`, `bg-transparent` 같은 유틸리티를 사용하세요:
...
색상 불투명도 수정자를 사용해 요소 배경색의 불투명도를 제어하세요:
...
완전히 사용자 정의한 값을 기반으로 배경색을 설정하려면 `bg-[<value>]` 문법을 사용하세요:

```
    <div class="bg-[#50d71e] ...">  <!-- ... --></div>
```

CSS 변수의 경우 `bg-(<custom-property>)` 문법도 사용할 수 있습니다:

```
    <div class="bg-(--my-color) ...">  <!-- ... --></div>
```

이는 `var()` 함수를 자동으로 추가해 주는 `bg-[var(<custom-property>)]`의 단축 문법입니다.

- hover 상태에 적용

`background-color` 유틸리티 앞에 `hover:*` 같은 variant를 붙이면 해당 상태에서만 유틸리티가 적용됩니다:
...
`background-color` 유틸리티 앞에 `md:` 같은 브레이크포인트 variant를 붙이면 중간 크기 이상의 화면에서만 유틸리티가 적용됩니다:
...
이제 마크업에서 `bg-regal-blue` 유틸리티를 사용할 수 있습니다:
